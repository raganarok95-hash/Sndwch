...
function sPComplaints(){
  ...
  return H('LIBRO DE RECLAMACIONES',"sc='"+bk+"';render()")+'<div style="flex:1;padding:24px 20px 40px;overflow-y:auto" class="fi">'
    ...
    +ST('01','TUS DATOS','')
    +INP('cq-name','NOMBRES Y APELLIDOS','text',undefined,'clientes')
    +INP('cq-dni','DNI / CARNET DE EXTRANJERÍA','text',undefined,'card')
    +INP('cq-addr','DOMICILIO','text',undefined,'direccion')
    +INP('cq-phone','TELÉFONO','tel',undefined,'phone')
    +INP('cq-email','CORREO ELECTRÓNICO','email',undefined,'mail')
    +minorBlock
    +'</div>'
...
function sAdminHome(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;background:#1E3932">'
    ...
    +'</div><button onclick="loadAdmin()" title="Actualizar ahora" aria-label="Actualizar ahora" style="all:unset;cursor:pointer;font-size:16px;margin-right:14px;display:inline-flex">'+icon('refresh',16)+'</button>'
    +'<button onclick="toggleAdminLight()" title="Modo claro/oscuro" aria-label="Cambiar modo claro/oscuro" style="all:unset;cursor:pointer;font-size:16px;margin-right:14px;display:inline-flex">'+icon(adminLightMode?'moon':'sun',16)+'</button>'
...
function sPRecover(){
  ...
  return H('RECUPERAR CUENTA',"sc='p_auth';render()")
    ...
    +INP('rec-phone','TELÉFONO // 9XXXXXXXX','tel',undefined,'phone')
    +INP('rec-dni','DNI // Tu número de 8 dígitos','text',undefined,'card')
    +INP('rec-bday','FECHA NACIMIENTO // DD/MM/AAAA','text',undefined,'calendar')
    +'</div>'
