...
function INP(id,ph,type?,val?,iconName?){
  ...
  return'<div style="position:relative">'
    ...
    +'<input id="'+id+'" type="'+(type||'text')+'" placeholder="'+ph+'" value="'+esc(val||'')+'" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#0d0d0d);border-radius:10px;padding:14px 16px 14px '+padLeft+';color:var(--sw-text,#FFFFFF);width:100%;font-size:16px;caret-color:'+GOLD+';box-shadow:'+SHADOW_SM+';box-sizing:border-box">'
    +'</div>';
}
...
function sOHome(){
  ...
  return H()
    ...
    +'<div onclick="startOrder(\'sig\')" style="background:'+'var(--sw-card2,#1A3028)'+';border:1px solid '+GOLD+';border-radius:14px;padding:24px 20px;cursor:pointer;margin-bottom:12px;position:relative;box-shadow:'+SHADOW_GOLD+'">'
    ...
    +(function(){
      ...
      +'<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:14px">'+visibleSigs.map(function(s){return'<span style="font-family:EB Garamond,serif;font-style:italic;font-size:11px;letter-spacing:.02em;color:'+GOLD+'">'+s.n+'</span>';}).join('  ·  ')+'</div>';
    })()
    +'</div>'
    +'<div onclick="startOrder(\'byo\')" style="background:'+'var(--sw-card,#2D5246)'+';border:1px solid var(--sw-border,#3A6B58);border-radius:14px;padding:22px 20px;cursor:pointer;margin-bottom:16px;box-shadow:'+SHADOW_SM+'">'
...
function sAdminHome(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;background:var(--sw-bg,#1E3932)">'
    ...
    +(ao.length?ao.map(function(o){
      ...
      return'<div style="background:var(--sw-card,#2D5246);border:1px solid '+(isStale?'#ff5555':(o.status==='RECIBIDO'?'rgba(255,165,0,.3)':'var(--sw-border-soft,#1c1c1c)'))+';border-radius:10px;padding:16px;margin-bottom:12px">'
