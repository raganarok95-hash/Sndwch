...
function renderOverlays(){
  ...
  if(sc.indexOf('admin')!==0&&sc!=='o_item_confirm'&&sc!=='o_cart'&&sc!=='o_home'){
  ...
  return H()
    ...
    +(function(){
      ...
      var officeCard='<div onclick="doCreateGroupOrder()" style="margin-top:14px;background:var(--sw-card2,#1A3028);border:1px solid rgba(203,162,88,.25);border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:12px;cursor:pointer">'
        ...
        +'<div style="font-family:\'EB Garamond\',serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px;line-height:1.4">Mandas un link, cada quien arma el suyo sin crear cuenta, y el delivery se divide entre todos.</div></div>'
...
function sAdminFocus(){
  ...
  var body='<div style="flex:1;padding:24px 20px 180px;overflow-y:auto" class="fi">'
    ...
    +(o.status==='EN CAMINO'&&o.eta_minutes?'<div onclick="event.stopPropagation();editEta(\''+o.id+'\','+o.eta_minutes+')" style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:11px;color:#3A86FF;margin-top:6px;display:flex;align-items:center;gap:6px;cursor:pointer">'+icon('moto',13,'#3A86FF')+'<span>ETA ~'+o.eta_minutes+' min · editar</span></div>':'')
...
async function pauseStore(minutes){
  ...
  try{
    ...
    render();
    setTimeout(function(){storeHoursMsg='';if(sc==='admin_hours')render();},3500);
  }catch(e){showToast(e.message);}
}
