...
var STATUSES={
  ...
  'CANCELADO': {c:'#8B8B8B',next:null,          icon:'close'}
};
...
function doLogout(){cust=null;isAdmin=false;savedPh='';token='';aErr='';clearGoogleLink();localStorage.removeItem('sw_ph');localStorage.removeItem('sw_tok');cacheCust(null);sc='p_auth';render();}
...
function bulkBar(){
  ...
  return'<div style="position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:480px;background:rgba(11,11,11,.97);border-top:1px solid #0B0B0B;padding:12px 16px;display:flex;gap:6px;align-items:center;z-index:110">'
    ...
    +'<button onclick="bulkSelected={};render()" style="all:unset;box-sizing:border-box;cursor:pointer;color:#ff8888;font-family:\'Barlow Condensed\',sans-serif;font-size:18px;width:40px;height:40px;display:flex;align-items:center;justify-content:center;flex-shrink:0">'+icon('close',16,'#ff8888')+'</button>'
    +'</div>';
}
...
function sAdminHome(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;background:#1E3932">'
    ...
    +'</div><button onclick="loadAdmin()" title="Actualizar ahora" style="all:unset;cursor:pointer;font-size:16px;margin-right:14px;display:inline-flex">'+icon('refresh',16)+'</button>'
    +'<button onclick="toggleAdminLight()" title="Modo claro/oscuro" style="all:unset;cursor:pointer;font-size:16px;margin-right:14px;display:inline-flex">'+icon(adminLightMode?'moon':'sun',16)+'</button>'
    ...
    +(ao.length?ao.map(function(o){
      ...
      return'<div style="background:#2D5246;border:1px solid '+(isStale?'#ff5555':(o.status==='RECIBIDO'?'rgba(255,165,0,.3)':'#1c1c1c'))+';border-radius:10px;padding:16px;margin-bottom:12px'+(isStale?'" class="pulse':'')+'">'
        ...
        +(o.receipt_path?'<button onclick="viewReceipt(\''+o.id+'\')" style="all:unset;cursor:pointer;display:block;width:100%;text-align:center;background:rgba(58,134,255,.12);border:1px solid rgba(58,134,255,.4);color:'+ACCENT2+';font-family:\'Barlow Condensed\',sans-serif;font-size:11px;font-weight:700;letter-spacing:.04em;padding:9px 4px;border-radius:8px;margin-bottom:8px">'+iconTxt('clip','VER COMPROBANTE',ACCENT2)+'</button>':'')
