...
function H(sub?,bk?,showCart?){
  ...
  var cartIcon=(showCart&&cart.length)?'<button onclick="go(\'o_cart\')" aria-label="Ver carrito" style="all:unset;cursor:pointer;position:relative;flex-shrink:0;padding:6px 10px;background:#2D5246;border-radius:8px;display:flex">'+icon('cart',18,'#F2F0EB')+'<span style="position:absolute;top:-4px;right:2px;background:'+GOLD+';color:#0d0d0d;font-family:\'Share Tech Mono\',monospace;font-size:9px;font-weight:700;border-radius:8px;padding:1px 5px;min-width:14px;text-align:center">'+cart.reduce(function(s,it){return s+it.qty;},0)+'</span></button>':'';
...
function AB(t,can?,bk?,nfn?,nl?){
  ...
  return'<div style="position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:480px;background:rgba(11,11,11,.97);border-top:1px solid #0B0B0B;box-shadow:0 -8px 24px rgba(0,0,0,.4);padding:12px 20px;display:flex;gap:10px;align-items:center;z-index:100"><div style="flex:1">'+tt+'</div>'+bb+'<button onclick="'+(can?nfn:'')+'" '+(can?'':'disabled')+' style="all:unset;cursor:'+(can?'pointer':'not-allowed')+';background:'+(can?surfaceGrad('#E0BC7A',GOLD):'#1E3932')+';box-shadow:'+(can?'0 3px 10px rgba(203,162,88,.3)':'none')+';color:'+(can?'#241a08':'#4A7A68')+';font-family:\'Barlow Condensed\',sans-serif;font-size:14px;font-weight:700;letter-spacing:.1em;padding:13px 0;border-radius:8px;text-align:center;flex:1">'+(nl||'CONTINUAR //')+'</button></div>';
}
...
function INP(id,ph,type?,val?,iconName?){
  ...
  return'<div style="position:relative">'
    ...
    +'<input id="'+id+'" type="'+(type||'text')+'" placeholder="'+ph+'" value="'+esc(val||'')+'" style="background:'+surfaceGrad('#2D5246','#213e33')+';border:1px solid #0d0d0d;border-radius:10px;padding:14px 16px 14px '+padLeft+';color:#FFFFFF;width:100%;font-size:16px;caret-color:'+GOLD+';box-shadow:'+SHADOW_SM+';box-sizing:border-box">'
    +'</div>';
}
...
function sOHome(){
  ...
  return H()
    ...
    +'</div>'
    +'<div onclick="startOrder(\'byo\')" style="background:'+surfaceGrad('#2D5246','#213e33')+';border:1px solid #3A6B58;border-radius:14px;padding:22px 20px;cursor:pointer;margin-bottom:16px;box-shadow:'+SHADOW_SM+'">'
...
function sOItemConfirm(){
  ...
  var sigNameHTML=(mode==='sig'&&sig)?'<div style="margin:2px 0 14px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:26px;font-weight:900;color:#FFFFFF;letter-spacing:.03em;line-height:1.15">'+sig.n+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+sigTypeTag(sig.s)+'</div></div>':'';
...
function checkoutExtrasHTML(){
  ...
  return pBox
    ...
    +'<div style="display:flex;flex-direction:column;gap:10px;margin-top:16px">'+INP('o-nom','NOMBRE // Tu nombre','text',confNom,'clientes')+INP('o-phone','TELÉFONO // 9XXXXXXXX','tel',confPhone,'phone')+INP('o-email','CORREO // Opcional, para tu comprobante','email',confEmail,'mail')+'<div style="position:relative">'+INP('o-addr','DIRECCION // Calle o usa GPS','text',addrText,'direccion')+'<button id="gps-btn" onclick="doGPS()" aria-label="Usar mi ubicación actual" style="all:unset;cursor:pointer;position:absolute;right:0;top:0;bottom:0;width:44px;display:flex;align-items:center;justify-content:center;color:#A8C8B0">'+icon('gps',16,'#A8C8B0')+'</button></div>'+'<div id="gps-hint" style="min-height:12px;margin-top:3px"></div>'+INP('o-notes','NOTAS // opcional','text',confNotes)+'</div>'
...
function payMethodBtn(id,label,enabled,badge){
  ...
  return'<div onclick="'+(enabled?'selectPayMethod(\''+id+'\')':'')+'" style="flex:1;text-align:center;background:'+(sel?surfaceGrad('#24543F','#173327'):surfaceGrad('#1E3A30','#162922'))+';border:1px solid '+(sel?GOLD:'#3A6B58')+';border-radius:8px;padding:10px 6px;cursor:'+(enabled?'pointer':'not-allowed')+';opacity:'+(enabled?1:.4)+'">'
    ...
    +(enabled?'':'<div style="font-family:\'Share Tech Mono\',monospace;font-size:7px;color:#A8C8B0;margin-top:2px;letter-spacing:.05em">PRONTO</div>')+'</div>';
}
...
function manualPayInstructionsHTML(t){
  ...
  return'<div style="margin-top:10px;background:#1A3028;border:1px solid '+GOLD+';border-radius:10px;padding:14px 16px">'
    ...
    +'<div style="font-family:\'Barlow\',sans-serif;font-size:10px;color:#A8C8B0;margin-top:12px;opacity:.85;display:flex;align-items:center;gap:5px">'+icon('lock',12,'#A8C8B0')+'<span>Nunca te pediremos tu clave, tu PIN ni un código que te llegue por SMS.</span></div>'
    +'</div>';
}
...
function sOSent(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px;text-align:center;background:#1E3932" class="fi">'
    ...
    +(deadlineLabel?'<div style="font-family:\'Barlow\',sans-serif;font-size:10px;color:#A8C8B0;margin-top:6px">Confirmamos manualmente — si no lo hacemos antes de las '+deadlineLabel+', el pedido se cancela solo.</div>':'')
...
function badgesHTML(c){
  ...
  return'<div style="background:#1A3028;border:1px solid rgba(203,162,88,.25);border-radius:12px;padding:18px;margin-bottom:16px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:700;color:#FFFFFF;margin-bottom:12px">INSIGNIAS<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+unlockedCount+'/'+badges.length+'</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">'+badges.map(function(b){
...
function sPProfile(){
  ...
  var balanceCompareHTML='<div style="font-family:\'Barlow\',sans-serif;font-size:10px;color:#8BAF9A;margin:4px 0 12px;line-height:1.5">Con tu saldo actual → <b style="color:#A8C8B0">crédito</b>. Con tus puntos → <b style="color:#A8C8B0">tarjeta de regalo</b>. Con tarjeta hoy → <b style="color:#A8C8B0">Plan Semanal</b>.</div>';
...
function sGiftCard(){
  ...
  return H('TARJETA DE REGALO','sc=\'p_profile\';render()')+'<div style="flex:1;padding:24px 20px 140px;overflow-y:auto" class="fi">'
    ...
    +'<p style="font-family:\'Barlow\',sans-serif;font-size:12px;color:#A8C8B0;margin-bottom:6px;line-height:1.5">Usa tus puntos para regalarle crédito SND//WCH a otro cliente al instante. Monto entre S/10 y S/500.</p>'
    ...
    +'</div>'
    +(ptsCost>0?'<div style="font-family:\'Barlow\',sans-serif;font-size:11px;color:#A8C8B0;margin-bottom:6px">Costará <b style="color:'+GOLD+'">'+ptsCost+' puntos</b>.</div>':'')
...
function sAdminHome(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;background:#1E3932">'
    ...
    +'<div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:24px;font-weight:900;color:#FFFFFF">PANEL<span class="cut-sep" style="color:'+GOLD+'"> // </span>OPERADOR</div>'
    ...
    +(ao.length?ao.map(function(o){
      ...
      return'<div style="background:#2D5246;border:1px solid '+(isStale?'#ff5555':(o.status==='RECIBIDO'?'rgba(255,165,0,.3)':'#1c1c1c'))+';border-radius:10px;padding:16px;margin-bottom:12px">'
        ...
        +(o.receipt_path?'<button onclick="viewReceipt(\''+o.id+'\')" style="all:unset;cursor:pointer;display:block;width:100%;text-align:center;background:rgba(168,200,176,.12);border:1px solid rgba(168,200,176,.4);color:#A8C8B0;font-family:\'Barlow Condensed\',sans-serif;font-size:11px;font-weight:700;letter-spacing:.04em;padding:9px 4px;border-radius:8px;margin-bottom:8px">'+iconTxt('clip','VER COMPROBANTE','#A8C8B0')+'</button>':'')
        ...
        +'</div>';
    }).join(''):'<div style="background:#2D5246;border:1px solid #2D5246;border-radius:10px;padding:24px 20px;text-align:center;margin-bottom:8px">'+icon('check',28,'#25D366')+'<div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:#A8C8B0;margin-top:8px">SIN PEDIDOS ACTIVOS //</div></div>')
...
function DTILE(label,big,sub?,muted?){
  ...
  var bg=muted?'#24382F':'#2D5246',bd=muted?'#2A473B':'#3A6B58',lc=muted?'#7FA08D':GOLD,bc=muted?'#C8D6CE':'#FFFFFF',bs=muted?'20px':'26px';
...
function sAdminDashboard(){
  ...
  if(alerts.length){
    h+='<div style="background:rgba(255,165,0,.1);border:1px solid rgba(255,165,0,.3);border-radius:10px;padding:14px 16px;margin-bottom:18px">'
      ...
      +alerts.map(function(a){return'<div style="font-family:Barlow,sans-serif;font-size:12px;color:#F2F0EB;margin-bottom:4px;display:flex;align-items:center;gap:5px">'+icon('warning',12,'#ffa500')+'<span>'+esc(a)+'</span></div>';}).join('')
      +'</div>';
  }
  // Ventas
  ...
  if(!stEntries.length){
    h+='<div style="font-family:Barlow,sans-serif;font-size:12px;color:#A8C8B0;margin-bottom:16px">Sin pedidos todavía.</div>';
  }else{
    ...
    if(liveEntries.length){
      ...
      h+='<div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#A8C8B0;letter-spacing:.1em;margin-bottom:6px">EN CURSO AHORA //</div>';
    ...
    if(doneEntries.length){
      ...
      h+='<div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#A8C8B0;letter-spacing:.1em;margin:10px 0 6px">HISTÓRICO //</div>';
  ...
  if(d.peakHours&&d.peakHours.length){
    ...
    h+='<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:#A8C8B0;margin-bottom:14px;display:flex;align-items:center;gap:5px"><span style="display:inline-flex;align-items:center;gap:5px">'+icon('reportes',12,'#A8C8B0')+'<span>Hora pico (últimos 90 días): </span></span><b style="color:#FFFFFF">'+peakTop.hour+':00-'+(peakTop.hour+1)+':00</b>'+(peakDayTop?' · Día pico: <b style="color:#FFFFFF">'+peakDayNames[peakDayTop.dow]+'</b>':'')+'</div>';
  }
  ...
  if(d.yapePlin&&d.yapePlin.total>0){
    ...
    h+='<div style="background:#2D5246;border:1px solid #3A6B58;border-radius:10px;padding:14px 16px;margin-bottom:14px"><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px">YAPE/PLIN — CONFIRMADOS VS. ABANDONADOS //</div><div style="display:flex;gap:8px"><div style="flex:1;text-align:center;background:#1A3028;border-radius:8px;padding:8px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:#25D366">'+d.yapePlin.confirmed+'</div><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#A8C8B0;letter-spacing:.05em">CONFIRMADOS</div></div><div style="flex:1;text-align:center;background:#1A3028;border-radius:8px;padding:8px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:#ff5555">'+d.yapePlin.abandoned+'</div><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#A8C8B0;letter-spacing:.05em">ABANDONADOS</div></div><div style="flex:1;text-align:center;background:#1A3028;border-radius:8px;padding:8px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:'+GOLD+'">'+ypRate+'%</div><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:#A8C8B0;letter-spacing:.05em">TASA CONFIRM.</div></div></div></div>';
  }
...
function render(){
  if(busy){
    ...
    if(sc.indexOf('admin')===0){appElBusy.innerHTML='<div style="min-height:100vh;background:#1E3932;padding:20px" class="fi">'+skeletonCards(4,64)+'</div>';}
...
function sAdminInventory(){
  ...
  INV_CATS.forEach(function(cat){
    h+='<div style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:900;color:#FFFFFF;margin:18px 0 10px">'+cat.t+'<span class="cut-sep" style="color:'+GOLD+'"> //</span></div>';
    h+=cat.arr.map(function(item){
      ...
      return'<div style="background:'+(av?'#2D5246':'#1A2420')+';border:1px solid '+(av?'#3A6B58':'rgba(255,85,85,.3)')+';border-radius:10px;padding:13px 16px;margin-bottom:8px">'
        ...
        +'<input id="qty-'+item.id+'" type="number" min="0" placeholder="Sin rastreo de cantidad" value="'+(tracked?qty:'')+'" style="flex:1;min-width:120px;background:#1A3028;border:1px solid #3A6B58;border-radius:8px;padding:9px 12px;color:#FFFFFF;font-size:16px;font-family:Share Tech Mono,monospace">'
...
function sAdminAudit(){
  ...
  h+=log.length?log.map(function(l){
    return'<div style="background:#2D5246;border:1px solid #3A6B58;border-radius:8px;padding:12px 14px;margin-bottom:8px">'
      +'<div style="display:flex;justify-content:space-between"><span style="font-family:Barlow Condensed,sans-serif;font-size:14px;font-weight:700;color:#FFFFFF">'+esc(AUDIT_ACTION_LABEL[l.action]||l.action)+'</span><span style="font-family:Share Tech Mono,monospace;font-size:9px;color:#A8C8B0">'+esc(l.actor_phone)+'</span></div>'
...
function sAdminComplaints(){
  ...
  h+=list.length?list.map(function(c){
    ...
    return'<div style="background:#2D5246;border:1px solid '+(pending?'rgba(255,165,0,.35)':'#3A6B58')+';border-radius:10px;padding:14px;margin-bottom:10px">'
      ...
      +'<div style="font-family:Barlow Condensed,sans-serif;font-size:15px;font-weight:700;color:#FFFFFF">'+esc(c.claim_code)+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+(c.kind==='queja'?'QUEJA':'RECLAMO')+'</div>'
