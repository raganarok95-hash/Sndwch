...
var SIGS=[
  ...
  // S/0 a 30CM frente a armarlo en BUILD YOUR OWN.
  {id:'SIG04',n:'THE FRESH',   s:'BUILD',    badge:'LIGERO',    base:'B01',prot:'P04',tops:['T01','T02','T06'],sauces:['S01','S11'],p15:16,p30:25,
  ...
  {id:'SIG05',n:'THE VAULT',   s:'RESERVE',  badge:'SECRETO',   base:'B03',prot:'P03',tops:['T04','T06','T03'],sauces:['S02','S12'],p15:24,p30:30,
    ...
    pitch:'Solo para clientes DE LA CASA. Pollo cajún, picante y con un golpe de miel — una combinación que no está en ningún menú, te la ganaste a pedidos.'}
];
...
// SIG07 (CHICAGO ITALIAN BEEF) usa una foto de stock real (Adobe Stock, licenciada,
...
function SZTOG(){
  ...
  return ST('00','TAMAÑO','Elige antes de continuar.')+'<div style="display:flex;gap:8px;margin-bottom:6px">'+opt('15','15CM','Individual')+opt('30','30CM','Clásico')+'</div><div style="height:1px;background:var(--sw-bg,#1E3932);margin:20px 0"></div>';
}
...
function sOSig(){
  ...
  var h=H('SIGNATURE BUILDS','go(\'o_home\')',true)+'<div style="flex:1;padding:20px 20px 140px;overflow-y:auto" class="fi">'+SZTOG()+recoCardSig+ST('01','ELIGE TU BUILD','Tres salsas incluidas.')+SIGS.map(function(s){
    ...
    // Antes, sin foto real (ver SIG_IMG — 4 de 6 signatures la tienen hoy), la tarjeta
    // simplemente no mostraba nada a la izquierda — el texto arrancaba pegado al borde
    // en esas filas mientras las demás tenían una miniatura, dando una lista "parchada"
    // (hallazgo del dueño). Mismo bloque de reserva que ya usa sigPreviewOverlayHTML
    // cuando no hay foto, a la misma escala que la miniatura real — nunca se inventa
    // una foto, solo se pareja el espacio que ocupa.
...
function sOItemConfirm(){
  ...
  var recU=(!doubleProt&&dbl)?{k:'doubleProt',e:icon('dumbbell',18,GOLD),l:'DOBLE PROTEÍNA',d:'El doble de tu proteína elegida',p:dbl.pDbl}:(!extraSauce&&sauceExtraAllowed)?{k:'sauce',e:icon('chili',18,GOLD),l:'SALSA EXTRA',d:'Salsa adicional a tu elección',p:2}:(cheeseSigAllowed&&!cheese)?{k:'cheese',e:icon('queso',18,GOLD),l:'QUESO',d:'Cheddar derretido, opcional y gratis',p:0}:null;
  ...
  return H('CONFIRMAR SÁNDWICH',(quickPayEligible?'backFromConfirm()':'go(\''+bk+'\')'),true)+'<div style="flex:1;padding:20px 20px 160px;overflow-y:auto" class="fi">'
    ...
    +'<details style="margin-bottom:12px"><summary style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;cursor:pointer;font-weight:700;list-style:none;padding:8px 0">TODOS LOS EXTRAS // ▾</summary><div style="margin-top:8px">'+(dbl?uBtn('doubleProt',icon('dumbbell',18,GOLD),'DOBLE PROTEÍNA','El doble de tu proteína elegida',dbl.pDbl,doubleProt):'')+(sauceExtraAllowed?uBtn('sauce',icon('chili',18,GOLD),'SALSA EXTRA','Salsa adicional a tu elección',2,extraSauce):'')+(cheeseSigAllowed?uBtn('cheese',icon('queso',18,GOLD),'QUESO','Cheddar derretido, opcional y gratis',0,!!cheese):'')+'</div></details>'
...
function checkoutExtrasHTML(){
  ...
  return pBox
    ...
    +'<div style="margin-top:10px;background:var(--sw-card2,#1A3028);border:1px solid rgba(203,162,88,.25);border-radius:10px;padding:12px 14px"><div style="font-family:\'Barlow\',sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);line-height:1.4;display:flex;align-items:flex-start;gap:8px">'+icon('moto',13,'#A8C8B0')+'<span>El costo de delivery se paga <b style="color:var(--sw-text,#FFFFFF)">directo al repartidor</b> al momento de la entrega — no está incluido en este total.</span></div></div>'
