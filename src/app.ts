...
// Sistema de profundidad compartido — antes tarjetas, botones y la barra inferior eran
// puro relleno plano sin ninguna sombra ni degradado, lo que leía más chato de lo que el
// resto del sistema visual (tipografía condensada, motivo "//", monospace, textura de
// grano en el fondo) ya lograba. Se centraliza acá para que la profundidad se sienta
// consistente en toda la app en vez de parchada pantalla por pantalla (auditoría estética
// de esta sesión, a pedido del dueño: "que estéticamente se vea más bonito").
var SHADOW_SM='0 2px 6px rgba(0,0,0,.22)';
var SHADOW_MD='0 4px 14px rgba(0,0,0,.28)';
var SHADOW_GOLD='0 4px 16px rgba(203,162,88,.15),0 2px 8px rgba(0,0,0,.3)';
...
var PROTS=[
  ...
  {id:'P06',l:'MEATBALL',s:'MARINARA',  d:'Albóndigas caseras en salsa marinara',p15:14,p30:24,pDbl:6}
];
var TOPS=[
  ...
  {id:'T07',l:'GIARDINIERA',s:'ENCURTIDO PICANTE',spicy:true}
];
var CHEESE=[
...
var SIGS=[
  ...
  {id:'SIG06',n:'THE TERIYAKI',s:'SIGNATURE',badge:'NUEVO',     base:'B01',prot:'P02',tops:['T01','T02','T06'],sauces:['S10','S05'],p15:17,p30:21,
  ...
  // escasez verificable.
  {id:'SIG07',n:'THE CHICAGO',s:'RESERVE',badge:'AL ESTILO CHICAGO',base:'B01',prot:'P01',tops:['T07'],sauces:['S13'],p15:25,p30:25,
  ...
  {id:'SIG05',n:'THE VAULT',   s:'RESERVE',  badge:'SECRETO',   base:'B03',prot:'P03',tops:['T04','T06','T03'],sauces:['S02','S12'],p15:24,p30:30,
    ...
    pitch:'Solo para clientes DE LA CASA. Una combinación que no está en ningún menú — te la ganaste a pedidos. No preguntes qué lleva. Pruébalo.'}
];
...
function scheduleTimePickerHTML(){
  ...
  var slotsHTML=slots.length
    ...
    :'<div style="margin-top:10px;font-family:\'Barlow\',sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0)">No hay horarios disponibles ese día.</div>';
...
function dblStockWarn(protId){
  ...
  return' <span style="color:#ffa500">— '+(q<=0?'sin stock ahora mismo':'solo queda '+q+', puede no alcanzar para doble')+'</span>';
}
...
// formulario sin enviarlo; sin este banner visible, un segundo cliente en el mismo tab que
...
function renderOverlays(){
  ...
  if(confirmState){
    html+='<div style="position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:410;display:flex;align-items:flex-end;justify-content:center" class="fi">'
      ...
      +'<p style="font-family:\'Barlow\',sans-serif;font-size:14px;color:var(--sw-text-body,#F2F0EB);line-height:1.5;margin-bottom:20px;white-space:pre-line">'+esc(confirmState.msg)+'</p>'
  ...
  if(promptState){
    html+='<div style="position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:420;display:flex;align-items:flex-end;justify-content:center" class="fi">'
      +'<div style="background:var(--sw-bg,#1E3932);border-radius:14px 14px 0 0;width:100%;max-width:420px;padding:24px 20px 20px;box-sizing:border-box">'
      +'<p style="font-family:\'Barlow\',sans-serif;font-size:14px;color:var(--sw-text-body,#F2F0EB);line-height:1.5;margin-bottom:14px;white-space:pre-line">'+esc(promptState.msg)+'</p>'
  ...
  if(sc.indexOf('admin')!==0&&sc!=='o_item_confirm'&&sc!=='o_cart'){
...
function sec(t,b){return'<div style="margin-bottom:20px"><div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:'+GOLD+';letter-spacing:.15em;margin-bottom:8px">'+t+'</div><p style="font-family:\'Barlow\',sans-serif;font-size:13px;color:var(--sw-text-muted,#A8C8B0);line-height:1.6">'+b+'</p></div>';}
...
function SZTOG(){
  function opt(sz,l,d){var sel=size===sz;return'<div onclick="size=\''+sz+'\';render()" style="flex:1;background:'+(sel?surfaceGrad('#24543F','#173327'):surfaceGrad('#1E3A30','#162922'))+';border:1px solid '+(sel?GOLD:'#3A6B58')+';border-radius:10px;padding:14px;cursor:pointer;text-align:center;position:relative;box-shadow:'+(sel?SHADOW_GOLD:SHADOW_SM)+'">'+selBar(sel)+'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:20px;font-weight:900;color:'+(sel?'#FFFFFF':'#A8C8B0')+'">'+l+'</div><div style="font-family:\'Barlow\',sans-serif;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+d+'</div></div>';}
...
function WORDMARK(size,hero?){
  return'<span style="font-family:\'Barlow Condensed\',sans-serif;font-size:'+size+'px;font-weight:900;color:#fff;letter-spacing:.02em;line-height:1'+(hero?';text-shadow:0 2px 14px rgba(0,0,0,.35)':'')+'">SND<span class="cut-sep" style="color:'+GOLD+';letter-spacing:.14em;padding:0 1px">//</span>WCH</span>';
}
function H(sub?,bk?,showCart?){
  var b=bk?'<button onclick="'+bk+'" style="all:unset;cursor:pointer;color:var(--sw-text-muted,#A8C8B0);font-family:\'Barlow Condensed\',sans-serif;font-size:18px;font-weight:700;padding:0 14px 0 0;flex-shrink:0">←</button>':'';
  var sz=sub?26:40;
...
function NAV(){
  var oa=tab==='order';
  function nb(t,l,a){return'<button onclick="swTab(\''+t+'\')" style="all:unset;cursor:pointer;flex:1;padding:12px 0;display:flex;flex-direction:column;align-items:center;gap:4px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;letter-spacing:.12em;color:'+(a?'#fff':'#555')+'">'+l+(a?' <span class="cut-sep" style="color:'+GOLD+'">//</span>':'')+'</div><div style="width:4px;height:4px;border-radius:50%;background:'+GOLD+';opacity:'+(a?1:0)+'"></div></button>';}
  return'<div style="position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:480px;background:rgba(11,11,11,.97);border-top:1px solid var(--sw-border-soft,#0B0B0B);display:flex;z-index:100">'+nb('order','PEDIDO',oa)+nb('points','PUNTOS',!oa)+'</div>';
}
...
function contactFooterHTML(){
  ...
  return'<div style="margin-top:28px;padding-top:20px;border-top:1px solid var(--sw-bg,#1E3932)">'
    ...
    +'<div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);line-height:2.1">'
    ...
    +'<span onclick="bkTo=\'o_home\';sc=\'p_legal\';render()" style="cursor:pointer;font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.06em;text-decoration:underline">TÉRMINOS //</span>'
    +'<span onclick="bkTo=\'o_home\';sc=\'p_returns\';render()" style="cursor:pointer;font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.06em;text-decoration:underline">CAMBIOS Y DEVOLUCIONES //</span>'
    +'<span onclick="bkTo=\'o_home\';sc=\'p_complaints\';cmplStep=\'form\';render()" style="cursor:pointer;font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.06em;text-decoration:underline">LIBRO DE RECLAMACIONES //</span>'
    +'</div>'
...
function AB(t,can?,bk?,nfn?,nl?){
  var bb=bk?'<button onclick="'+bk+'" style="all:unset;cursor:pointer;border:1px solid var(--sw-border,#3A6B58);color:var(--sw-text-muted,#A8C8B0);font-family:\'Barlow Condensed\',sans-serif;font-size:12px;font-weight:700;letter-spacing:.1em;padding:12px 16px;border-radius:8px;flex-shrink:0">← ATRÁS</button>':'';
  var tt=t?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:8px;color:'+GOLD+';letter-spacing:.2em">TOTAL //</div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:24px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+SOLES+'<span style="color:'+GOLD+'">'+t+'</span></div>':'';
  ...
  // El botón principal lleva un degradado + resplandor dorado sutil cuando está activo,
  // en vez de relleno plano, para que se lea como LA acción de la pantalla.
  return'<div style="position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:480px;background:rgba(11,11,11,.97);border-top:1px solid var(--sw-border-soft,#0B0B0B);box-shadow:0 -8px 24px rgba(0,0,0,.4);padding:12px 20px;display:flex;gap:10px;align-items:center;z-index:100"><div style="flex:1">'+tt+'</div>'+bb+'<button onclick="'+(can?nfn:'')+'" '+(can?'':'disabled')+' style="all:unset;cursor:'+(can?'pointer':'not-allowed')+';background:'+(can?surfaceGrad('#E0BC7A',GOLD):'#1E3932')+';box-shadow:'+(can?'0 3px 10px rgba(203,162,88,.3)':'none')+';color:'+(can?'#241a08':'#4A7A68')+';font-family:\'Barlow Condensed\',sans-serif;font-size:14px;font-weight:700;letter-spacing:.1em;padding:13px 0;border-radius:8px;text-align:center;flex:1">'+(nl||'CONTINUAR //')+'</button></div>';
}
...
function CARD(item,sel,fn,right?,thumb?){
  var inner='<div style="display:flex;justify-content:space-between;align-items:center"><span style="font-family:\'Barlow Condensed\',sans-serif;font-size:16px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+item.l+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+item.s+'</span>'+(right||'')+'</div>'+(item.d?'<p style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">'+item.d+'</p>':'');
  return'<div onclick="'+fn+'" style="background:'+(sel?surfaceGrad('#235643','#193c2e'):surfaceGrad('#1d3830','#162a22'))+';border:1px solid '+(sel?GOLD:'#3A6B58')+';border-radius:10px;padding:14px 16px;cursor:pointer;margin-bottom:10px;position:relative;transition:all .15s;box-shadow:'+(sel?SHADOW_GOLD:SHADOW_SM)+'">'+selBar(sel)+(thumb?'<div style="display:flex;gap:14px">'+thumb+'<div style="flex:1;min-width:0">'+inner+'</div></div>':inner)+'</div>';
}
function ST(n,t,s?){return'<div style="margin-bottom:20px"><h2 style="font-family:\'Barlow Condensed\',sans-serif;font-size:28px;font-weight:900;color:#fff;letter-spacing:.06em;line-height:1">'+n+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+t+'</h2>'+(s?'<p style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:5px">'+s+'</p>':'')+'</div>';}
...
function BTN(l,fn,out?){return'<button onclick="'+fn+'" style="all:unset;box-sizing:border-box;cursor:pointer;display:block;width:100%;background:'+(out?'transparent':surfaceGrad('#E0BC7A',GOLD))+';border:'+(out?'1px solid #A8C8B0':'none')+';box-shadow:'+(out?'none':'0 3px 10px rgba(203,162,88,.3)')+';color:'+(out?'#A8C8B0':'#241a08')+';font-family:\'Barlow Condensed\',sans-serif;font-size:14px;font-weight:700;letter-spacing:.1em;padding:14px;border-radius:10px;text-align:center">'+l+'</button>';}
function LOAD(msg){return'<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:var(--sw-bg,#1E3932)"><div style="margin-bottom:16px">'+WORDMARK(38,true)+'</div><div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.25em">'+(msg||'CARGANDO //')+'</div></div>';}
...
function sOHome(){
  ...
  return H()
    ...
    +cartCard
    +recoCard
    +'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-body,#F2F0EB);opacity:.8;letter-spacing:.15em;margin-bottom:20px">\xbfCÓMO QUIERES PEDIR? //</div>'
    // SIGNATURE BUILDS es el camino recomendado (rápido, sin decisiones) — se le da un
    // tratamiento "hero" real (degradado propio + resplandor dorado) en vez de compartir
    // el mismo relleno plano que BUILD YOUR OWN, para que la jerarquía se sienta con solo
...
function sGroupOrder(){
  ...
  h+='<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:24px;font-weight:900;color:#fff;margin-bottom:4px">PEDIDO<span class="cut-sep" style="color:'+GOLD+'"> // </span>GRUPAL</div>';
  ...
  h+=(g.items.length?g.items.map(function(it){
    return'<div style="background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:12px 14px;margin:10px 0 0;display:flex;justify-content:space-between;align-items:center"><div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:14px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(it.label)+'</div><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0)">'+esc(it.contributorName)+'</div></div><div style="font-family:\'Share Tech Mono\',monospace;font-size:13px;color:'+GOLD+'">'+SOLES+it.unitPrice+'</div></div>';
  }).join(''):'<div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:14px">Nadie agregó su pedido todavía.</div>');
  h+='<div style="display:flex;justify-content:space-between;align-items:center;background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin:16px 0"><span style="font-family:\'Barlow Condensed\',sans-serif;font-size:14px;font-weight:700;color:var(--sw-text-body,#F2F0EB)">TOTAL</span><span style="font-family:\'Barlow Condensed\',sans-serif;font-size:24px;font-weight:900;color:'+GOLD+'">'+SOLES+g.total+'</span></div>';
  if(g.status!=='open'){
    h+='<div style="text-align:center;font-family:\'Share Tech Mono\',monospace;font-size:11px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.1em">'+(g.status==='cancelled'?'ESTE PEDIDO GRUPAL FUE CANCELADO':'ESTE PEDIDO GRUPAL YA SE CERRÓ')+'</div>';
  }else{
  ...
  var h=H('SIGNATURE BUILDS','go(\'o_home\')',true)+'<div style="flex:1;padding:20px 20px 140px;overflow-y:auto" class="fi">'+SZTOG()+recoCardSig+ST('01','ELIGE TU BUILD','Tres salsas incluidas.')+SIGS.map(function(s){
    ...
    if(!av){
      ...
      return'<div style="background:var(--sw-card-danger,#1A2420);border:1px solid rgba(255,85,85,.3);border-radius:10px;padding:16px;margin-bottom:10px;opacity:.7"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px"><span style="font-family:\'Barlow Condensed\',sans-serif;font-size:18px;font-weight:700;color:var(--sw-text-muted,#A8C8B0)">'+s.n+'<span style="color:var(--sw-text-muted,#A8C8B0)"> // </span>'+sigTypeTag(s.s)+'</span><span style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#ff8888">AGOTADO</span></div><div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:8px">'+(bs?bs.l+' // '+bs.s:'')+' · '+(pr?pr.l+' // '+pr.s:'')+'</div>'
        ...
        // UX, MEDIO).
        +(cust?'<button onclick="doRequestRestockNotify(\''+s.id+'\')" '+(notifyRequested?'disabled':'')+' style="all:unset;cursor:'+(notifyRequested?'default':'pointer')+';display:block;margin-top:10px;font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+(notifyRequested?'#25D366':GOLD)+';letter-spacing:.08em">'+(notifyRequested?'✓ TE AVISAMOS CUANDO VUELVA':'AVÍSAME CUANDO VUELVA →')+'</button>':'<div onclick="swTab(\'points\')" style="cursor:pointer;display:block;margin-top:10px;font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.08em">INICIA SESIÓN PARA QUE TE AVISEMOS →</div>')
        +'</div>';
    }
...
function sigPreviewOverlayHTML(){
  ...
  var hero=photo
    ...
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:24px;font-weight:900;color:#fff">'+s.n+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+sigTypeTag(s.s)+'</div>'
  ...
  return'<div onclick="closeSigPreview()" style="position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:200;display:flex;align-items:flex-end;justify-content:center" class="fi">'
    ...
    +'<p style="font-family:\'Barlow\',sans-serif;font-size:13px;color:var(--sw-text-body,#F2F0EB);line-height:1.6;margin-bottom:16px">'+esc(s.pitch||'')+'</p>'
    ...
    +(s.secret
      ?'<div style="background:var(--sw-card,#2D5246);border:1px solid rgba(203,162,88,.35);border-radius:10px;padding:14px 16px;margin-bottom:16px;text-align:center"><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:6px">INGREDIENTES //</div><div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);font-style:italic">Secretos. Se revelan cuando lo pruebas.</div></div>'
      :'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:16px"><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px">INGREDIENTES //</div>'
      +'<div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);line-height:1.8">'
    ...
    +'<div onclick="closeSigPreview()" style="text-align:center;cursor:pointer;font-family:\'Share Tech Mono\',monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.1em;padding:4px">CERRAR</div>'
    +'</div></div></div>';
}
...
function sOBuild(){
  ...
  var progressBar='<div style="display:flex;gap:4px;margin-bottom:10px">'+BYO_STEP_LABELS.map(function(_,i){
    ...
    return'<div style="flex:1;height:5px;border-radius:4px;background:'+(st==='todo'?'#2D5246':GOLD)+';box-shadow:'+(st==='current'?'0 0 8px rgba(203,162,88,.6)':'none')+';opacity:'+(st==='todo'?1:st==='done'?.55:1)+'"></div>';
  }).join('')+'</div>';
  var stepLabel='<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.15em;margin-bottom:18px">PASO '+(byoStep+1)+' // 5</div>';
  var h=H('BUILD YOUR OWN','byoStepBack()',true)+'<div style="flex:1;padding:20px 20px 160px;overflow-y:auto" class="fi">'+progressBar+stepLabel;
  if(byoStep===0){
    h+=SZTOG();
    h+=ST('01','PAN','');
...
// SIDES/BEBIDAS
function sOSides(){
  var h=H('BEBIDAS Y SIDES',"sc='o_cart';render()",true)+'<div style="flex:1;padding:20px 20px 140px;overflow-y:auto" class="fi">'+ST('01','ELIGE','Se agregan a tu carrito.');
...
function cartItemsHTML(){
  ...
  return cart.map(function(it,idx){
    ...
    return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:8px"><div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px"><div style="flex:1"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(itemLabel(it))+'</div>'+(extras?'<div style="font-family:\'Barlow\',sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(extras)+'</div>':'')+'</div><div style="display:flex;gap:10px;flex-shrink:0">'+(canEdit?'<button onclick="editCartItem('+idx+')" style="all:unset;cursor:pointer;color:'+GOLD+';font-family:\'Share Tech Mono\',monospace;font-size:10px">EDITAR</button>':'')+'<button onclick="cartRemove('+idx+')" style="all:unset;cursor:pointer;color:#ff8888;font-family:\'Share Tech Mono\',monospace;font-size:10px">QUITAR</button></div></div>'+(canEdit?'<div onclick="editItemNote('+idx+')" style="cursor:pointer;margin-top:4px;font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0)">'+(it.note?icon('reclamo',11,'#A8C8B0')+'<span style="margin-left:5px">'+esc(it.note)+'</span>':'+ agregar nota (ej. sin cebolla)')+'</div>':'')+'<div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px"><div style="display:flex;align-items:center;gap:10px"><button onclick="cartQtyChange('+idx+',-1)" style="all:unset;cursor:pointer;width:34px;height:34px;background:var(--sw-card2,#1A3028);border-radius:6px;text-align:center;color:var(--sw-text,#FFFFFF);font-family:\'Barlow Condensed\',sans-serif;font-size:16px;font-weight:700">−</button><span class="bump" style="display:inline-block;font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF);min-width:16px;text-align:center">'+it.qty+'</span><button onclick="cartQtyChange('+idx+',1)" style="all:unset;cursor:pointer;width:34px;height:34px;background:var(--sw-card2,#1A3028);border-radius:6px;text-align:center;color:var(--sw-text,#FFFFFF);font-family:\'Barlow Condensed\',sans-serif;font-size:16px;font-weight:700">+</button></div><span style="font-family:\'Share Tech Mono\',monospace;font-size:13px;color:'+GOLD+'">'+SOLES+itemLineTotal(it)+'</span></div></div>';
  }).join('');
}
...
function rewardsPickerHTML(){
  ...
  if(!unlocked.length){
    ...
    return'<div style="margin-top:16px;background:var(--sw-card2,#1A3028);border:1px solid rgba(203,162,88,.2);border-radius:8px;padding:12px 14px"><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.15em;margin-bottom:4px">TUS PUNTOS //</div><div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0)">Te faltan <span style="color:var(--sw-text-body,#F2F0EB);font-weight:700">'+missingPts+' pts</span> para '+cheapest.n+' // '+cheapest.s+'.</div></div>';
  }
  var rows=unlocked.map(function(r){
    ...
    return'<div onclick="'+(eligible?'toggleReward(\''+r.id+'\')':'')+'" style="background:'+(selected?surfaceGrad('#24543F','#173327'):surfaceGrad('#1E3A30','#162922'))+';border:1px solid '+(selected?GOLD:'#3A6B58')+';border-radius:10px;padding:12px 14px;margin-bottom:8px;cursor:'+(eligible?'pointer':'not-allowed')+';opacity:'+(eligible?1:.4)+';box-shadow:'+(selected?SHADOW_GOLD:SHADOW_SM)+'"><div style="display:flex;justify-content:space-between;align-items:center"><div style="flex:1;padding-right:8px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:14px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+r.n+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+r.s+'</div><div style="font-family:\'Barlow\',sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(sub)+'</div></div><span style="font-family:\'Share Tech Mono\',monospace;font-size:16px;color:'+(selected?GOLD:'#A8C8B0')+';flex-shrink:0">'+(selected?'✓':'○')+'</span></div></div>';
  }).join('');
...
function payMethodBtn(id,label,enabled,badge){
  ...
  return'<div onclick="'+(enabled?'selectPayMethod(\''+id+'\')':'')+'" style="flex:1;text-align:center;background:'+(sel?surfaceGrad('#24543F','#173327'):surfaceGrad('#1E3A30','#162922'))+';border:1px solid '+(sel?GOLD:'#3A6B58')+';border-radius:8px;padding:10px 6px;cursor:'+(enabled?'pointer':'not-allowed')+';opacity:'+(enabled?1:.4)+'">'
    ...
    +'<div style="display:flex;align-items:center;justify-content:center;font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;color:#fff">'+label+'</div>'
    +(enabled?'':'<div style="font-family:\'Share Tech Mono\',monospace;font-size:7px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px;letter-spacing:.05em">PRONTO</div>')+'</div>';
}
...
function payStep(n,label,body){
  return'<div style="display:flex;gap:10px;margin-top:'+(n===1?'0':'10px')+'"><div style="flex:0 0 auto;width:20px;height:20px;border-radius:50%;background:'+GOLD+';color:#0d0d0d;font-family:\'Barlow Condensed\',sans-serif;font-size:11px;font-weight:900;display:flex;align-items:center;justify-content:center">'+n+'</div><div style="flex:1;min-width:0"><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.1em;margin-bottom:4px">'+label+'</div>'+body+'</div></div>';
}
function manualPayInstructionsHTML(t){
  ...
  return'<div style="margin-top:10px;background:var(--sw-card2,#1A3028);border:1px solid '+GOLD+';border-radius:10px;padding:14px 16px">'
    +'<div style="text-align:center;margin-bottom:14px"><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.2em;margin-bottom:2px">MONTO A TRANSFERIR //</div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:38px;font-weight:900;color:'+GOLD+'">'+SOLES+t+'</div></div>'
    +payStep(1,'TRANSFIERE POR YAPE O PLIN A','<div style="display:flex;justify-content:space-between;align-items:center;background:var(--sw-card,#2D5246);border-radius:8px;padding:8px 10px"><div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:900;color:#fff">'+YAPE_PLIN_PHONE+'</div><div style="font-family:\'Share Tech Mono\',monospace;font-size:8px;color:var(--sw-text-muted,#A8C8B0)">'+esc(YAPE_PLIN_NAME)+(recurring?' · ya usaste este número antes ✓':'')+'</div></div></div>'
    ...
    +payStep(2,'CONFIRMA AQUÍ ABAJO','<div style="font-family:\'Barlow\',sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);line-height:1.4">Toca "YA REALICÉ EL PAGO". Tu pedido pasa a cocina recién cuando lo verifiquemos.</div>')
    +'<div style="font-family:\'Barlow\',sans-serif;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-top:12px;opacity:.85;display:flex;align-items:center;gap:5px">'+icon('lock',12,'#A8C8B0')+'<span>Nunca te pediremos tu clave, tu PIN ni un código que te llegue por SMS.</span></div>'
    +'</div>';
}
...
function sOCart(){
  ...
  return H('TU CARRITO',"syncConfirmFields();sc='o_home';render()")+'<div style="flex:1;padding:20px 20px 160px;overflow-y:auto" class="fi">'
    ...
    +'<div style="display:flex;gap:8px;margin-bottom:20px"><div onclick="syncConfirmFields();go(\'o_home\')" style="flex:1;text-align:center;background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:12px;cursor:pointer;font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;color:var(--sw-text,#FFFFFF)">+ SÁNDWICH</div><div onclick="syncConfirmFields();sc=\'o_sides\';render()" style="flex:1;text-align:center;background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:12px;cursor:pointer;font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;color:var(--sw-text,#FFFFFF)">+ BEBIDA/SIDE</div></div>'
...
function sOSent(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px;text-align:center;background:var(--sw-bg,#1E3932)" class="fi">'
    ...
    +stBadge('RECIBIDO')
    +(manualWaiting?'<div style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:10px"><span class="pulse" style="width:7px;height:7px;border-radius:50%;background:'+GOLD+';display:inline-block"></span><span style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.1em">ESPERANDO CONFIRMACIÓN DE PAGO</span></div>':'')
    +(deadlineLabel?'<div style="font-family:\'Barlow\',sans-serif;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-top:6px">Confirmamos manualmente — si no lo hacemos antes de las '+deadlineLabel+', el pedido se cancela solo.</div>':'')
    +(manualWaiting?'<div style="margin-top:10px">'
      +(receiptUploadState==='done'
        ?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#25D366">✓ comprobante recibido — gracias</div>'
        :'<label style="display:inline-flex;align-items:center;gap:6px;cursor:'+(receiptUploadState==='uploading'?'default':'pointer')+';font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';text-decoration:underline;letter-spacing:.05em">'
      ...
      +(typeof receiptUploadState==='string'&&receiptUploadState.indexOf('error:')===0?'<div style="font-family:\'Barlow\',sans-serif;font-size:10px;color:#ff8888;margin-top:4px">'+esc(receiptUploadState.slice(6))+'</div>':'')
      +'</div>':'')
    +'<div style="font-family:\'Barlow\',sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-top:10px;line-height:1.5">Sigue el estado en PUNTOS → MIS PEDIDOS</div></div>'
    +'<div style="background:var(--sw-card2,#1A3028);border:1px solid rgba(37,211,102,.25);border-radius:12px;padding:14px 20px;margin-bottom:16px;width:100%;max-width:320px"><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#25D366;letter-spacing:.2em;margin-bottom:4px">'+(pending?'MONTO A PAGAR //':(window._lTot===0?'CUBIERTO POR RECOMPENSA //':'MONTO COBRADO //'))+'</div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:32px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+SOLES+(window._lTot||0)+'</div>'+(window._lChargeId?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:8px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">Ref. pago: '+window._lChargeId+'</div>':'')+'</div>'
...
function sPAuth(){
  return H()+'<div style="flex:1;padding:24px 20px 140px;overflow-y:auto" class="fi"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:30px;font-weight:900;color:#fff;margin-bottom:6px">PUNTOS<span class="cut-sep" style="color:'+GOLD+'"> // </span>REWARDS</div><p style="font-family:\'Barlow\',sans-serif;font-size:13px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:24px;line-height:1.6">Acumula puntos con cada pedido. Canjéalos por salsas, upgrades y sándwiches gratis. Bono de bienvenida: +40 pts al crear tu cuenta.</p><div style="display:flex;background:var(--sw-card,#2D5246);border-radius:10px;padding:4px;margin-bottom:24px">'+[['reg','CREAR CUENTA'],['login','INGRESAR']].map(function(x){return'<button onclick="clearGoogleLink();atab=\''+x[0]+'\';aErr=\'\';render()" style="all:unset;cursor:pointer;flex:1;background:'+(atab===x[0]?GOLD:'transparent')+';color:'+(atab===x[0]?'#fff':'#555')+';font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;letter-spacing:.1em;padding:11px 0;border-radius:8px;text-align:center;transition:all .15s">'+x[1]+'</button>';}).join('')+'</div>'+(googleConfigured()?'<div id="google-btn-mount" style="display:flex;justify-content:center;margin-bottom:14px;min-height:44px"></div><div style="display:flex;align-items:center;gap:10px;margin-bottom:18px"><div style="flex:1;height:1px;background:var(--sw-card,#2D5246)"></div><span style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#5A7A6A;letter-spacing:.15em">O CON TU TELÉFONO</span><div style="flex:1;height:1px;background:var(--sw-card,#2D5246)"></div></div>':'')+(atab==='reg'?'<div style="display:flex;flex-direction:column;gap:10px">'+(_googleIdToken?'<div style="background:rgba(203,162,88,.12);border:1px solid rgba(203,162,88,.3);border-radius:10px;padding:12px 14px;margin-bottom:4px;display:flex;flex-direction:column;gap:6px"><div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-body,#F2F0EB);line-height:1.4">✓ Verificamos <b>'+esc(_googleLinkedEmail||'')+'</b> con Google. Completa tu registro para vincularla — si no eres tú, descarta este vínculo abajo.</div><div onclick="discardGoogleLink()" style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);cursor:pointer;text-decoration:underline;align-self:flex-start">No soy yo — continuar sin Google</div></div>':'')+INP('r-name','NOMBRE // Tu nombre completo','text',window._lastGuestName||'','clientes')+INP('r-phone','TELÉFONO // 9XXXXXXXX','tel',window._lastGuestPhone||'','phone')+INP('r-pin','PIN PERSONAL // Mínimo 4 dígitos','password',undefined,'lock')+INP('r-email','CORREO // Para recuperar tu cuenta','email',window._lastGuestEmail||'','mail')+INP('r-dni','DNI // 8 dígitos (obligatorio)','text',undefined,'card')+INP('r-bday','FECHA NACIMIENTO // DD/MM/AAAA','text',undefined,'calendar')+INP('r-ref','CÓDIGO DE REFERIDO // opcional','text',refCode)+'<div id="auth-err" style="font-family:\'Barlow\',sans-serif;font-size:12px;color:#ff5555;min-height:16px">'+aErr+'</div>'+'<p style="font-family:\'Barlow\',sans-serif;font-size:10px;color:var(--sw-text-muted,#A8C8B0);line-height:1.5;margin-bottom:4px">Al crear tu cuenta aceptas nuestros <span onclick="event.stopPropagation();sc=\'p_legal\';render()" style="color:'+GOLD+';cursor:pointer;text-decoration:underline">Términos y Política de Privacidad</span>.</p>'+BTN('CREAR CUENTA //','doReg()')+'</div>':'<div style="display:flex;flex-direction:column;gap:10px">'+INP('l-phone','TELÉFONO // 9XXXXXXXX','tel',savedPh,'phone')+INP('l-pin','PIN PERSONAL','password',undefined,'lock')+'<div id="auth-err" style="font-family:\'Barlow\',sans-serif;font-size:12px;color:#ff5555;min-height:16px">'+aErr+'</div>'+BTN('INGRESAR //','doLogin()')+' '+`<div onclick="recNewPin=null;recEmailMasked=null;recPhone='';recDni='';recBday='';sc='p_recover';render()" style="text-align:center;margin-top:10px;font-family:'Share Tech Mono',monospace;font-size:10px;color:'+GOLD+';cursor:pointer;letter-spacing:.1em">¿Olvidaste tu PIN? // Recuperar →</div>`+'</div>')+'<div style="margin-top:28px;border-top:1px solid var(--sw-border-soft,#0B0B0B);padding-top:20px"><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:12px">RECOMPENSAS //</div>'+RWDS.map(function(r){return'<div style="display:flex;justify-content:space-between;margin-bottom:10px"><span style="font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:700;color:var(--sw-text-muted,#A8C8B0)">'+r.n+'<span style="color:var(--sw-text-muted,#A8C8B0)"> // </span>'+r.s+'</span><span style="font-family:\'Share Tech Mono\',monospace;font-size:11px;color:var(--sw-text-muted,#A8C8B0)">'+r.pts+' pts</span></div>';}).join('')+'</div></div>'+NAV();
}
async function doReg(){
  ...
  var bday=null;
  if(bdayRaw){
    bday=parseBdayDDMMYYYY(bdayRaw);
    if(!bday){if(err)err.textContent='Fecha de nacimiento inválida — debe ser DD/MM/AAAA y existir de verdad.';return;}
  }
...
function sWelcome(){
  ...
  return'<div onclick="sc=\'p_home\';render()" style="min-height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;background:var(--sw-bg,#1E3932);padding:48px 24px;position:relative;overflow:hidden">'
    ...
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:48px;font-weight:900;color:var(--sw-text-body,#F2F0EB);line-height:1;margin-bottom:4px">'+esc(nm.toUpperCase())+'</div>'
    ...
    +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">PUNTOS ACUMULADOS</div>'
    ...
    +(next?'<div style="margin-top:8px"><div style="font-family:\'Barlow\',sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0)">Te faltan <span style="color:var(--sw-text-body,#F2F0EB);font-weight:700">'+(next.pts-pts)+' pts</span> para '+next.n+' // '+next.s+'</div></div>':'')
    +'</div>'
    ...
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:var(--sw-text-body,#F2F0EB)">'+(cust?cust.total_orders||0:0)+'</div>'
    +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:7px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">PEDIDOS</div></div>'
    +'<div style="background:rgba(242,240,235,.08);border:1px solid rgba(242,240,235,.12);border-radius:12px;padding:12px 16px;text-align:center">'
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:var(--sw-text-body,#F2F0EB)">'+(cust?cust.total_redeemed||0:0)+'</div>'
    +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:7px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">CANJEADOS</div></div>'
    +'</div>'
    ...
    +'</div>'
    +'<div style="margin-top:20px;font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-body,#F2F0EB);letter-spacing:.15em;opacity:.7" class="blink">toca para continuar //</div>'
    +'</div></div>';
}
function sPHome(){
...
function sPOrders(){
  ...
  function card(o){
    ...
    return'<div onclick="_od=\''+o.id+'\';rtStars=0;rtMsg=\'\';sc=\'p_ord_detail\';render()" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:12px;padding:14px;margin-bottom:10px;cursor:pointer">'
      +'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px">'
      +'<div><div style="font-family:Barlow Condensed,sans-serif;font-size:16px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(o.customer_name)+'</div>'
      +'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(o.ref)+' · '+esc(o.date)+'</div></div>'
      ...
      +'<div style="font-family:Barlow,sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:8px">'+esc(o.summary)+'</div>'
  ...
  var h=H('MIS PEDIDOS',"sc='p_home';render()")+'<div style="flex:1;padding:20px 20px 140px;overflow-y:auto" class="fi">';
  if(listLoading){
...
function sOrdDetail(){
  ...
  return H('DETALLE',"sc='p_orders';render()")+'<div style="flex:1;padding:20px 20px 40px;overflow-y:auto" class="fi">'
    ...
    +'<div><div style="font-family:Barlow Condensed,sans-serif;font-size:22px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+esc(o.customer_name)+'</div>'
    +'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:'+GOLD+';margin-top:2px">'+esc(o.ref)+'</div>'
    +'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(o.date)+'</div></div>'
    +'<div style="font-family:Barlow Condensed,sans-serif;font-size:34px;font-weight:900;color:'+GOLD+'">'+SOLES+o.total+'</div></div>'
    +'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:4px">PEDIDO //</div>'
    +'<div style="font-family:Barlow,sans-serif;font-size:13px;color:#ddd;line-height:1.6;margin-bottom:10px">'+esc(o.summary)+'</div>'
    +'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:4px">DIRECCIÓN //</div>'
    +'<div style="font-family:Barlow,sans-serif;font-size:12px;color:#aaa">'+esc(o.customer_address)+'</div>'
    +(o.redeemed_reward?'<div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--sw-border,#3A6B58)"><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:#25D366;display:flex;align-items:center;gap:6px">'+icon('gift',12,'#25D366')+esc(o.redeemed_reward)+'</div></div>':'')
    +'</div>'
    ...
    +'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:10px">ESTADO //</div>'
    +'<div style="display:flex;gap:4px;margin-bottom:12px">'+STEPS.map(function(st,i){var dn=i<=ci;return'<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:3px"><div style="height:5px;width:100%;background:'+(dn?GOLD:'#3A6B58')+';border-radius:4px"></div><div style="font-family:Share Tech Mono,monospace;font-size:7px;color:'+(dn?GOLD:'#4A7A68')+';text-align:center;line-height:1.3">'+st.replace(' ','<br>')+'</div></div>';}).join('')+'</div>'
    +stBadge(o.status)
    +'</div>'
    ...
    +(o.status!=='ENTREGADO'?'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);text-align:center;margin-top:10px" class="blink">&#8635; Toca ACTUALIZAR en MIS PEDIDOS</div>':'<div style="font-family:Barlow,sans-serif;font-size:13px;color:#25D366;text-align:center;margin-top:10px">&#9989; ¡Entregado!</div>')
    +ratingHTML(o)
    +'</div>'+NAV();
}
...
function sPHistory(){
  ...
  var h=H('HISTORIAL DE PUNTOS',"sc='p_home';render()")+'<div style="flex:1;padding:20px 20px 140px;overflow-y:auto" class="fi">';
  if(listLoading){
    ...
    h+=txns.map(function(t){
      ...
      return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-family:Barlow,sans-serif;font-size:13px;color:var(--sw-text-body,#F2F0EB)">'+esc(t.description||t.type)+'</div><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(t.date||'')+'</div></div><div style="font-family:Barlow Condensed,sans-serif;font-size:20px;font-weight:900;color:'+(pos?'#25D366':'#ff8888')+'">'+(pos?'+':'')+t.points+'</div></div>';
    }).join('');
  }
  h+='</div>'+NAV();
  return h;
...
function badgesHTML(c){
  ...
  return'<div style="background:var(--sw-card2,#1A3028);border:1px solid rgba(203,162,88,.25);border-radius:12px;padding:18px;margin-bottom:16px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF);margin-bottom:12px">INSIGNIAS<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+unlockedCount+'/'+badges.length+'</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">'+badges.map(function(b){
    return'<div style="background:'+(b.unlocked?'#1E4A38':'#0d1a15')+';border:1px solid '+(b.unlocked?GOLD:'#2a2a2a')+';border-radius:10px;padding:10px;text-align:center;opacity:'+(b.unlocked?1:.4)+'"><div>'+icon(b.icon,22,(b.unlocked?GOLD:'#666'))+'</div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:11px;font-weight:700;color:'+(b.unlocked?'#FFFFFF':'#A8C8B0')+';margin-top:4px">'+b.label+'</div>'+(b.sub?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:8px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+b.sub+'</div>':'')+'</div>';
  }).join('')+'</div></div>';
}
function sPProfile(){
  var initial=esc((cust.name||'?').trim().charAt(0).toUpperCase());
  var heroHTML='<div style="background:linear-gradient(135deg,#2D5246,#1E3932);border:1px solid var(--sw-border,#3A6B58);border-radius:16px;padding:22px;margin-bottom:16px;display:flex;align-items:center;gap:16px"><div style="flex:0 0 auto;width:56px;height:56px;border-radius:50%;background:'+GOLD+';display:flex;align-items:center;justify-content:center;font-family:\'Barlow Condensed\',sans-serif;font-size:26px;font-weight:900;color:#12241D">'+initial+'</div><div style="flex:1;min-width:0"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:24px;font-weight:900;color:var(--sw-text,#FFFFFF);line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(cust.name)+'</div><div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">'+esc(cust.phone)+'</div><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.15em;margin-top:6px">'+rankName(cust.total_orders)+' //</div></div><div style="flex:0 0 auto;text-align:center;background:rgba(0,0,0,.2);border-radius:10px;padding:8px 12px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:20px;font-weight:900;color:'+GOLD+';line-height:1">'+(cust.points||0)+'</div><div style="font-family:\'Share Tech Mono\',monospace;font-size:8px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.1em;margin-top:2px">PTS</div></div></div>';
  var referralHTML='<div style="background:var(--sw-card2,#1A3028);border:1px solid rgba(203,162,88,.25);border-radius:12px;padding:18px;margin-bottom:16px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF);margin-bottom:10px">PROGRAMA<span class="cut-sep" style="color:'+GOLD+'"> // </span>REFERIDOS</div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:26px;font-weight:900;color:var(--sw-text,#FFFFFF);margin-bottom:4px">'+cust.phone+'</div><div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:12px">Tu código de referido · '+(cust.total_referrals||0)+' amigos referidos</div><button onclick="shareReferral()" style="all:unset;cursor:pointer;display:block;width:100%;background:'+GOLD+';color:#0d0d0d;font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;letter-spacing:.08em;padding:12px;border-radius:8px;text-align:center">COMPARTIR POR WHATSAPP //</button></div>';
  ...
  var creditHTML='<div style="background:var(--sw-card2,#1A3028);border:1px solid rgba(203,162,88,.25);border-radius:12px;padding:18px;margin-bottom:16px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF);margin-bottom:10px">CRÉDITO<span class="cut-sep" style="color:'+GOLD+'"> // </span>SND//WCH</div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:34px;font-weight:900;color:var(--sw-text,#FFFFFF);margin-bottom:4px">'+SOLES+(cust.credit_balance||0)+'</div><div style="font-family:\'Barlow\',sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:14px;line-height:1.5">No es dinero real: no se retira ni se transfiere a un banco. Solo sirve para pagar pedidos o regalarlo a otro cliente SND//WCH.</div><div style="display:flex;flex-direction:column;gap:8px">'+INP('cg-phone','TELÉFONO DEL AMIGO // 9XXXXXXXX','tel',wPhone,'phone')+INP('cg-amt','MONTO A REGALAR // S/','number',wAmt,'coin')+'<div id="cg-msg" style="font-family:\'Barlow\',sans-serif;font-size:11px;color:'+GOLD+';min-height:14px">'+wMsg+'</div>'+BTN('REGALAR CRÉDITO //','doCreditGift()',true)+'</div></div>';
...
function sGiftCard(){
  ...
  return H('TARJETA DE REGALO','sc=\'p_profile\';render()')+'<div style="flex:1;padding:24px 20px 140px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:#fff;margin-bottom:4px">TARJETA<span class="cut-sep" style="color:'+GOLD+'"> // </span>DE REGALO</div>'
    +'<p style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:6px;line-height:1.5">Usa tus puntos para regalarle crédito SND//WCH a otro cliente al instante. Monto entre S/10 y S/500.</p>'
    +'<p style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:'+GOLD+';letter-spacing:.05em;margin-bottom:18px">TIENES '+(cust.points||0)+' PTS // '+GIFT_CARD_POINTS_PER_SOL+' PTS = S/1</p>'
    +'<div style="display:flex;flex-direction:column;gap:8px;margin-bottom:6px">'
    +INP('gc-phone','TELÉFONO DEL DESTINATARIO // 9XXXXXXXX','tel',gcPhone,'phone')
    +INP('gc-amt','MONTO // S/','number',gcAmt,'coin')
    +'</div>'
    +(ptsCost>0?'<div style="font-family:\'Barlow\',sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:6px">Costará <b style="color:'+GOLD+'">'+ptsCost+' puntos</b>.</div>':'')
...
function sWeeklyPlan(){
  return H('PLAN SEMANAL','sc=\'p_profile\';render()')+'<div style="flex:1;padding:24px 20px 140px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:#fff;margin-bottom:4px">PLAN<span class="cut-sep" style="color:'+GOLD+'"> // </span>SEMANAL</div>'
    +'<p style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:20px;line-height:1.5">Paga '+SOLES+WEEKLY_PLAN_PRICE+' hoy con tu tarjeta y recibe '+SOLES+WEEKLY_PLAN_CREDIT+' en saldo SND//WCH al instante — pide cuando quieras esta semana, el saldo no vence.</p>'
    +'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:16px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:center"><span style="font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;color:var(--sw-text-body,#F2F0EB)">PAGAS HOY</span><span style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+SOLES+WEEKLY_PLAN_PRICE+'</span></div>'
    +'<div style="background:rgba(37,211,102,.1);border:1px solid rgba(37,211,102,.3);border-radius:10px;padding:16px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:center"><span style="font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;color:var(--sw-text-body,#F2F0EB)">RECIBES EN SALDO</span><span style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:#25D366">'+SOLES+WEEKLY_PLAN_CREDIT+'</span></div>'
...
function doLogout(){
  ...
  myOrders=[];myAddresses=[];myFavorites=[];pickedAddrId=null;
...
function sPLegal(){
  ...
  return H('TÉRMINOS Y PRIVACIDAD',"sc='"+bk+"';render()")+'<div style="flex:1;padding:24px 20px 40px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:#fff;margin-bottom:4px">TÉRMINOS<span class="cut-sep" style="color:'+GOLD+'"> // </span>Y PRIVACIDAD</div>'
    +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:20px">Última actualización: 2026</div>'
    +providerBlockHTML()
...
function providerBlockHTML(){
  return'<div style="background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:20px;font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);line-height:1.9">'
...
function sPReturns(){
  ...
  return H('CAMBIOS Y DEVOLUCIONES',"sc='"+bk+"';render()")+'<div style="flex:1;padding:24px 20px 40px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:#fff;margin-bottom:4px">CAMBIOS<span class="cut-sep" style="color:'+GOLD+'"> // </span>Y DEVOLUCIONES</div>'
    +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:20px">Última actualización: 2026</div>'
    +providerBlockHTML()
...
function sPComplaints(){
  ...
  var kindToggle='<div style="display:flex;background:var(--sw-card,#2D5246);border-radius:10px;padding:4px;margin-bottom:20px">'+[['reclamo','RECLAMO'],['queja','QUEJA']].map(function(x){return'<button onclick="cmplKind=\''+x[0]+'\';render()" style="all:unset;cursor:pointer;flex:1;background:'+(cmplKind===x[0]?GOLD:'transparent')+';color:'+(cmplKind===x[0]?'#fff':'#555')+';font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;letter-spacing:.1em;padding:11px 0;border-radius:8px;text-align:center;transition:all .15s">'+x[1]+'</button>';}).join('')+'</div>';
  ...
  return H('LIBRO DE RECLAMACIONES',"sc='"+bk+"';render()")+'<div style="flex:1;padding:24px 20px 40px;overflow-y:auto" class="fi">'
    ...
    +'<p style="font-family:\'Barlow\',sans-serif;font-size:10px;color:var(--sw-text-muted,#A8C8B0);line-height:1.5;margin-top:14px">Tenemos hasta 30 días calendario para responder tu reclamo o queja, conforme a la normativa vigente.</p>'
    +'</div>';
}
function sComplaintsSuccess(bk){
  return H('LIBRO DE RECLAMACIONES',"sc='"+bk+"';render()")+'<div style="flex:1;padding:24px 20px 40px;overflow-y:auto;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center" class="fi">'
    ...
    +'<p style="font-family:\'Barlow\',sans-serif;font-size:13px;color:var(--sw-text-muted,#A8C8B0);line-height:1.6;max-width:320px">Te enviamos una copia a tu correo. Responderemos dentro de los 30 días calendario siguientes, conforme a ley.</p>'
...
function sPFavorites(){
  var h=H('MIS FAVORITOS',"sc='p_home';render()")+'<div style="flex:1;padding:20px 20px 140px;overflow-y:auto" class="fi">';
...
function sPAddresses(){
  var h=H('MIS DIRECCIONES',"sc='p_home';render()")+'<div style="flex:1;padding:20px 20px 140px;overflow-y:auto" class="fi">';
  if(myAddresses.length){
    h+=myAddresses.map(function(a){
      return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-family:Barlow Condensed,sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(a.label)+'</div><div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(a.address)+'</div></div><button onclick="doDeleteAddress(\''+a.id+'\')" style="all:unset;cursor:pointer;color:#ff8888;font-family:Share Tech Mono,monospace;font-size:10px;flex-shrink:0;margin-left:10px">ELIMINAR</button></div>';
    }).join('');
  }else{
  ...
  h+='<div style="margin-top:20px;background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:16px"><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:'+GOLD+';letter-spacing:.15em;margin-bottom:10px">AGREGAR DIRECCIÓN //</div><div style="display:flex;flex-direction:column;gap:8px">'+INP('na-label','NOMBRE // Casa, Trabajo...','text',undefined,'clientes')+INP('na-addr','DIRECCIÓN COMPLETA','text',undefined,'direccion')+'<div id="na-msg" style="font-family:Barlow,sans-serif;font-size:11px;color:#ff5555;min-height:14px">'+newAddrMsg+'</div>'+BTN('GUARDAR DIRECCIÓN //','doAddAddress()')+'</div></div>';
  h+='</div>'+NAV();
  return h;
}
...
function bulkBar(){
  ...
  return'<div style="position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:480px;background:rgba(11,11,11,.97);border-top:1px solid var(--sw-border-soft,#0B0B0B);padding:12px 16px;display:flex;gap:6px;align-items:center;z-index:110">'
    ...
    +'<button onclick="bulkConfirmPayments()" style="all:unset;box-sizing:border-box;cursor:pointer;flex:1;text-align:center;background:#ffa500;color:#0d0d0d;font-family:\'Barlow Condensed\',sans-serif;font-size:12px;font-weight:700;padding:15px 4px;border-radius:8px">'+iconTxt('check','PAGO','#0d0d0d')+'</button>'
...
function sAdminHome(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;background:var(--sw-bg,#1E3932)">'
    ...
    +'<div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:24px;font-weight:900;color:var(--sw-text,#FFFFFF)">PANEL<span class="cut-sep" style="color:'+GOLD+'"> // </span>OPERADOR</div>'
    +(badge>0?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#ffa500;letter-spacing:.1em;margin-top:3px" class="pulse">● '+badge+' ACCIÓN REQUERIDA</div>':'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:3px">todo en orden //</div>')
    ...
    +(adminOrdersTruncated?'<div style="background:rgba(255,165,0,.12);border-bottom:1px solid rgba(255,165,0,.3);padding:8px 20px;font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#ffa500;display:flex;align-items:center;gap:5px">'+icon('warning',12,'#ffa500')+'<span>Hay más pedidos activos de los que se muestran aquí (solo los '+ao.length+' más recientes).</span></div>':'')
    ...
    // de funcionar.
    +(pollFailing?'<div style="background:rgba(255,85,85,.12);border-bottom:1px solid rgba(255,85,85,.3);padding:8px 20px;font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#ff8888;display:flex;align-items:center;gap:5px">'+icon('warning',12,'#ff8888')+'<span>No se pudo actualizar la cola de pedidos — reintentando…</span></div>':'')
    ...
    +(ao.length?ao.map(function(o){
      ...
      return'<div style="background:var(--sw-card,#2D5246);border:1px solid '+(isStale?'#ff5555':(o.status==='RECIBIDO'?'rgba(255,165,0,.3)':'#1c1c1c'))+';border-radius:10px;padding:16px;margin-bottom:12px">'
        ...
        +'<div style="flex:1"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:17px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(o.customer_name)+'</div>'
        +'<div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(o.customer_address)+'</div>'
        +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+(isStale?'#ff8888':'#A8C8B0')+';margin-top:4px;display:flex;align-items:center;gap:5px">'+(isStale?'<span class="pulse" style="width:6px;height:6px;border-radius:50%;background:#ff5555;display:inline-block;flex-shrink:0"></span>':'')+'<span>'+esc(o.ref)+' · '+SOLES+o.total+' · '+esc(o.date)+(mins!==null?' · hace '+mins+' min':'')+'</span></div>'
        +(isScheduledAhead?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#3A86FF;margin-top:2px;display:flex;align-items:center;gap:5px">'+icon('horario',12,'#3A86FF')+'<span>programado para '+esc(new Date(o.scheduled_for).toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'}))+'</span></div>':'')
        ...
        +(o.status==='EN CAMINO'&&o.eta_minutes?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#3A86FF;margin-top:2px;display:flex;align-items:center;gap:5px">'+icon('moto',12,'#3A86FF')+'<span>ETA ~'+o.eta_minutes+' min</span></div>':'')+'</div></div>'
        +stBadge(o.status)+'</div>'
        +'<div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:12px">'+esc(o.summary)+'</div>'
        +(o.redeemed_reward?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#25D366;margin-bottom:10px;display:flex;align-items:center;gap:5px">'+icon('gift',12,'#25D366')+'<span>'+esc(o.redeemed_reward)+'</span></div>':'')
        +(o.payment_method==='cod'&&o.payment_status!=='paid'?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#ffa500;margin-bottom:10px;display:flex;align-items:center;gap:5px">'+icon('cash',12,'#ffa500')+'<span>COBRAR '+SOLES+o.total+' AL ENTREGAR</span></div>':'')
        +(manualPending?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:#ffa500;margin-bottom:8px;display:flex;align-items:center;gap:5px">'+icon('warning',12,'#ffa500')+'<span>PAGO '+manualLabel+' SIN CONFIRMAR — revisa tu app antes de continuar</span></div>':'')
        +(o.payment_status==='paid'&&PAYMENT_METHOD_BADGE[o.payment_method]?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:var(--sw-text-muted2,#8BAF9A);margin-bottom:10px">'+PAYMENT_METHOD_BADGE[o.payment_method]+'</div>':'')
        ...
        +(o.receipt_path?'<button onclick="viewReceipt(\''+o.id+'\')" style="all:unset;cursor:pointer;display:block;width:100%;text-align:center;background:rgba(168,200,176,.12);border:1px solid rgba(168,200,176,.4);color:var(--sw-text-muted,#A8C8B0);font-family:\'Barlow Condensed\',sans-serif;font-size:11px;font-weight:700;letter-spacing:.04em;padding:9px 4px;border-radius:8px;margin-bottom:8px">'+iconTxt('clip','VER COMPROBANTE','#A8C8B0')+'</button>':'')
        ...
        +'<button onclick="printTicket(\''+o.id+'\')" style="all:unset;cursor:pointer;flex:1;text-align:center;background:rgba(139,175,154,.12);border:1px solid rgba(139,175,154,.4);color:var(--sw-text-muted,#A8C8B0);font-family:\'Barlow Condensed\',sans-serif;font-size:11px;font-weight:700;letter-spacing:.04em;padding:9px 4px;border-radius:8px">'+iconTxt('printer','TICKET','#A8C8B0')+'</button>'
        +((o.contact_phone||o.customer_phone)?'<button onclick="waAdmin(\''+o.id+'\')" style="all:unset;cursor:pointer;flex:1;text-align:center;background:rgba(203,162,88,.12);border:1px solid rgba(203,162,88,.4);color:'+GOLD+';font-family:\'Barlow Condensed\',sans-serif;font-size:11px;font-weight:700;letter-spacing:.04em;padding:9px 4px;border-radius:8px">'+iconTxt('chat','WHATSAPP',GOLD)+'</button>':'')
        +'</div>'
        +(manualPending
          ?'<button onclick="confirmAndAdvance(\''+o.id+'\')" style="all:unset;cursor:pointer;display:block;width:100%;background:#ffa500;color:#0d0d0d;font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;letter-spacing:.06em;padding:11px 0;border-radius:8px;text-align:center;margin-bottom:6px">'+iconTxt('check','CONFIRMAR PAGO Y PREPARAR','#0d0d0d')+'</button>'
            +'<button onclick="confirmOrderPayment(\''+o.id+'\')" style="all:unset;cursor:pointer;display:block;width:100%;text-align:center;color:var(--sw-text-muted2,#8BAF9A);font-family:\'Barlow\',sans-serif;font-size:10px;padding:6px 0;margin-bottom:8px">solo confirmar el pago, sin avanzar todavía</button>'
          :(s.next?'<button onclick="updateStatus(\''+o.id+'\',\''+s.next+'\')" style="all:unset;cursor:pointer;display:block;width:100%;background:'+STATUSES[s.next].c+';color:var(--sw-text,#FFFFFF);font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;letter-spacing:.06em;padding:11px 0;border-radius:8px;text-align:center">'+(STATUSES[s.next].icon&&ICONS[STATUSES[s.next].icon]?icon(STATUSES[s.next].icon,13,'#fff')+' ':'')+'MARCAR COMO '+s.next+' →</button>':'<div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:#25D366;text-align:center;padding:8px">'+iconTxt('check','COMPLETADO','#25D366')+'</div>'))
        ...
        +'<button onclick="cancelOrder(\''+o.id+'\')" style="all:unset;cursor:pointer;display:block;width:100%;background:transparent;border:1px solid rgba(255,85,85,.4);color:#ff8888;font-family:\'Barlow Condensed\',sans-serif;font-size:12px;font-weight:700;letter-spacing:.06em;padding:9px 0;border-radius:8px;text-align:center">'+iconTxt('close','CANCELAR PEDIDO'+(manualPending?' (nunca pagó)':''),'#ff8888')+'</button>'
        +'</div>';
    }).join(''):'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-card,#2D5246);border-radius:10px;padding:24px 20px;text-align:center;margin-bottom:8px">'+icon('check',28,'#25D366')+'<div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-top:8px">SIN PEDIDOS ACTIVOS //</div></div>')
    ...
    +(!cust||!('serviceWorker' in navigator)||!('PushManager' in window)?'':'<div onclick="togglePushNotifications()" style="margin-top:18px;background:var(--sw-card,#2D5246);border:1px solid '+(pushSubscribed?GOLD:'#1c1c1c')+';border-radius:10px;padding:12px 16px;cursor:pointer"><div style="display:flex;justify-content:space-between;align-items:center"><span style="display:inline-flex;align-items:center;gap:8px;font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;color:var(--sw-text-muted,#A8C8B0)">'+icon('notif')+'ALERTAS DE PEDIDOS Y STOCK</span><span style="font-family:\'Share Tech Mono\',monospace;font-size:14px;color:'+(pushSubscribed?GOLD:'#A8C8B0')+'">'+(pushSubscribed?'✓ ACTIVO':'○ ACTIVAR')+'</span></div>'+(pushMsg?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:'+GOLD+';margin-top:6px">'+esc(pushMsg)+'</div>':'')+'</div>')
    +'<div style="margin-top:18px;background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#1c1c1c);border-radius:10px;padding:12px 16px;display:flex;justify-content:space-between;align-items:center"><span style="display:inline-flex;align-items:center;gap:8px;font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;color:var(--sw-text-muted,#A8C8B0)">'+icon('sonido')+'SONIDO DE NUEVO PEDIDO</span>'
    +'<select onchange="setNotifSound(this.value)" style="background:var(--sw-bg,#1E3932);color:var(--sw-text,#FFFFFF);border:1px solid var(--sw-border,#3A6B58);border-radius:6px;padding:6px 8px;font-family:\'Share Tech Mono\',monospace;font-size:10px">'
...
function DTILE(label,big,sub?,muted?){
  ...
  return'<div style="background:'+bg+';border:1px solid '+bd+';border-radius:12px;padding:14px 16px"><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:'+lc+';letter-spacing:.15em;margin-bottom:6px">'+label+'</div><div style="font-family:Barlow Condensed,sans-serif;font-size:'+bs+';font-weight:900;color:'+bc+';line-height:1">'+big+'</div>'+(sub?'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">'+sub+'</div>':'')+'</div>';
}
...
function DHERO(label,big,sub?){
  return'<div style="background:linear-gradient(135deg,rgba(203,162,88,.16),rgba(203,162,88,.05));border:1px solid rgba(203,162,88,.5);border-radius:14px;padding:20px 18px;box-shadow:0 4px 22px rgba(203,162,88,.14)"><div style="font-family:Share Tech Mono,monospace;font-size:10px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:8px;font-weight:700">'+label+'</div><div style="font-family:Barlow Condensed,sans-serif;font-size:42px;font-weight:900;color:var(--sw-text,#FFFFFF);line-height:1">'+big+'</div>'+(sub?'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:'+GOLD+';margin-top:6px">'+sub+'</div>':'')+'</div>';
}
function DBAR(label,value,max,color?){
  var pct=max>0?Math.min(100,Math.round((value/max)*100)):0;
  return'<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-body,#F2F0EB)">'+label+'</span><span style="font-family:Share Tech Mono,monospace;font-size:11px;color:'+(color||GOLD)+'">'+value+'</span></div><div style="background:var(--sw-bg,#1E3932);border-radius:4px;height:8px;overflow:hidden"><div style="background:'+(color||GOLD)+';height:100%;width:'+pct+'%;border-radius:4px"></div></div></div>';
}
function sAdminDashboard(){
  ...
  if(alerts.length){
    h+='<div style="background:rgba(255,165,0,.1);border:1px solid rgba(255,165,0,.3);border-radius:10px;padding:14px 16px;margin-bottom:18px">'
      ...
      +alerts.map(function(a){return'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-body,#F2F0EB);margin-bottom:4px;display:flex;align-items:center;gap:5px">'+icon('warning',12,'#ffa500')+'<span>'+esc(a)+'</span></div>';}).join('')
      +'</div>';
  }
  // Ventas
  ...
  if(d.trend&&d.trend.length){
    ...
    h+='<div style="display:flex;gap:3px;margin-bottom:18px">'+d.trend.map(function(t,i){return'<div style="flex:1;text-align:center;font-family:Share Tech Mono,monospace;font-size:6px;color:var(--sw-text-muted,#A8C8B0)">'+(i%2===0?t.date:'')+'</div>';}).join('')+'</div>';
  }
  // Pedidos por estado
  ...
  if(!stEntries.length){
    h+='<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:16px">Sin pedidos todavía.</div>';
  }else{
    ...
    if(liveEntries.length){
      ...
      h+='<div style="font-family:Share Tech Mono,monospace;font-size:8px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.1em;margin-bottom:6px">EN CURSO AHORA //</div>';
    ...
    if(doneEntries.length){
      ...
      h+='<div style="font-family:Share Tech Mono,monospace;font-size:8px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.1em;margin:10px 0 6px">HISTÓRICO //</div>';
  ...
  if(d.peakHours&&d.peakHours.length){
    ...
    h+='<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:14px;display:flex;align-items:center;gap:5px"><span style="display:inline-flex;align-items:center;gap:5px">'+icon('reportes',12,'#A8C8B0')+'<span>Hora pico (últimos 90 días): </span></span><b style="color:var(--sw-text,#FFFFFF)">'+peakTop.hour+':00-'+(peakTop.hour+1)+':00</b>'+(peakDayTop?' · Día pico: <b style="color:var(--sw-text,#FFFFFF)">'+peakDayNames[peakDayTop.dow]+'</b>':'')+'</div>';
  }
  ...
  if(d.yapePlin&&d.yapePlin.total>0){
    ...
    h+='<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:14px"><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px">YAPE/PLIN — CONFIRMADOS VS. ABANDONADOS //</div><div style="display:flex;gap:8px"><div style="flex:1;text-align:center;background:var(--sw-card2,#1A3028);border-radius:8px;padding:8px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:#25D366">'+d.yapePlin.confirmed+'</div><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.05em">CONFIRMADOS</div></div><div style="flex:1;text-align:center;background:var(--sw-card2,#1A3028);border-radius:8px;padding:8px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:#ff5555">'+d.yapePlin.abandoned+'</div><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.05em">ABANDONADOS</div></div><div style="flex:1;text-align:center;background:var(--sw-card2,#1A3028);border-radius:8px;padding:8px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:'+GOLD+'">'+ypRate+'%</div><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.05em">TASA CONFIRM.</div></div></div></div>';
  }
  ...
  if(d.bySource&&d.bySource.length){
    h+='<div style="margin-bottom:16px"><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px">ORIGEN DE CLIENTES //</div>'
      +d.bySource.map(function(s: any){return'<div style="display:flex;justify-content:space-between;align-items:center;background:var(--sw-card,#2D5246);border-radius:8px;padding:8px 12px;margin-bottom:6px"><span style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-body,#F2F0EB)">'+esc(s.source)+'</span><span style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0)">'+s.signups+' registrados · <span style="color:'+GOLD+'">'+s.converted+' pidieron</span></span></div>';}).join('')
      +'</div>';
  }
  ...
  if(atRiskCustomers&&atRiskCustomers.length){
    h+=atRiskCustomers.slice(0,10).map(function(c){
      ...
      return'<div style="display:flex;justify-content:space-between;align-items:center;background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:10px 14px;margin-bottom:8px"><div><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:13px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(c.name||c.phone)+'</div><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(c.rank)+' · último pedido '+daysTxt+'</div></div><button onclick="waRiskContact(\''+esc(c.phone)+'\',\''+esc(c.name||'')+'\')" aria-label="Contactar por WhatsApp" style="all:unset;cursor:pointer;background:rgba(203,162,88,.12);border:1px solid rgba(203,162,88,.4);color:'+GOLD+';font-family:\'Barlow Condensed\',sans-serif;font-size:11px;font-weight:700;padding:8px 12px;border-radius:8px;flex-shrink:0;display:inline-flex">'+icon('chat',14,GOLD)+'</button></div>';
    }).join('');
  }else{
    h+='<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:14px">Sin clientes en riesgo por ahora.</div>';
  }
  h+='<div style="height:1px;background:var(--sw-bg,#1E3932);margin:16px 0"></div>';
  // Puntos
...
// ADMIN GEN
function sAdminGen(){return'<div style="min-height:100vh;display:flex;flex-direction:column;background:var(--sw-bg,#1E3932)"><div style="padding:20px;border-bottom:1px solid #3A6B58;display:flex;align-items:center;gap:12px"><button onclick="sc=\'admin_home\';render()" style="all:unset;cursor:pointer;color:var(--sw-text-muted,#A8C8B0);font-family:\'Barlow Condensed\',sans-serif;font-size:18px;font-weight:700;padding:0 12px 0 0">←</button><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:22px;font-weight:900;color:var(--sw-text,#FFFFFF)">PUNTOS<span class="cut-sep" style="color:'+GOLD+'"> // </span>MANUALES</div></div><div style="flex:1;padding:24px 20px" class="fi"><p style="font-family:\'Barlow\',sans-serif;font-size:14px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:20px;line-height:1.6">Otorga puntos confirmados directamente a un cliente.</p><div style="display:flex;flex-direction:column;gap:12px;margin-bottom:16px">'+INP('ag-ph','TELÉFONO DEL CLIENTE // 9XXXXXXXX','tel',agPhone,'phone')+INP('ag-pts','PUNTOS A OTORGAR // Ej: 25','number',agPts)+BTN('OTORGAR PUNTOS //','doManualPts()')+'</div><div id="ag-msg" style="font-family:\'Barlow\',sans-serif;font-size:13px;color:'+GOLD+';min-height:20px">'+agMsg+'</div>'
  +'<div style="height:1px;background:var(--sw-card,#2D5246);margin:28px 0 24px"></div>'
  +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:18px;font-weight:900;color:var(--sw-text,#FFFFFF);margin-bottom:4px">CRÉDITO<span class="cut-sep" style="color:'+GOLD+'"> // </span>AJUSTE MANUAL</div>'
  +'<p style="font-family:\'Barlow\',sans-serif;font-size:14px;color:var(--sw-text-muted,#A8C8B0);margin:8px 0 20px;line-height:1.6">Corrige el saldo de crédito interno de un cliente (ej. reponer un pedido cancelado que se pagó con crédito). Puede ser negativo para descontar un exceso otorgado por error.</p><div style="display:flex;flex-direction:column;gap:12px">'+INP('ac-ph','TELÉFONO DEL CLIENTE // 9XXXXXXXX','tel',acPhone,'phone')+INP('ac-delta','MONTO // positivo suma, negativo descuenta','number',acDelta,'coin')+BTN('AJUSTAR CRÉDITO //','doManualCredit()')+'</div><div id="ac-msg" style="font-family:\'Barlow\',sans-serif;font-size:13px;color:'+GOLD+';min-height:20px;margin-top:12px">'+acMsg+'</div></div></div>';}
...
function render(){
  ...
  var offlineBanner=isOffline?'<div style="background:#ffa500;color:#1a1200;text-align:center;padding:6px;font-family:\'Share Tech Mono\',monospace;font-size:10px;letter-spacing:.1em;font-weight:700;display:flex;align-items:center;justify-content:center;gap:5px">'+icon('warning',12,'#1a1200')+'<span>SIN CONEXIÓN — reconectando…</span></div>':'';
  ...
  INV_CATS.forEach(function(cat){
    ...
    h+=cat.arr.map(function(item){
      ...
      return'<div class="inv-row" data-name="'+esc(name.toLowerCase())+'" style="background:'+(av?'var(--sw-card,#2D5246)':'var(--sw-card-danger,#1A2420)')+';border:1px solid '+(av?'var(--sw-border,#3A6B58)':'rgba(255,85,85,.3)')+';border-radius:10px;padding:13px 16px;margin-bottom:8px">'
        ...
        +'<input id="qty-'+item.id+'" type="number" min="0" placeholder="Sin rastreo de cantidad" value="'+(tracked?qty:'')+'" style="flex:1;min-width:120px;background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:9px 12px;color:var(--sw-text,#FFFFFF);font-size:16px;font-family:Share Tech Mono,monospace">'
...
function sAdminCustomer(){
  ...
  if(custDetail){
    ...
    h+='<div style="height:1px;background:var(--sw-bg,#1E3932);margin:20px 0"></div>'
      ...
      +'<div style="font-family:Barlow Condensed,sans-serif;font-size:20px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+esc(c.name)+'</div>'
      +'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">'+esc(c.phone)+(c.email?' · '+esc(c.email):'')+'</div>'
      +'<div style="display:flex;gap:16px;margin-top:10px;flex-wrap:wrap">'
      +'<div><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:'+GOLD+'">PUNTOS</div><div style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+(c.points||0)+'</div></div>'
      +'<div><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:'+GOLD+'">PEDIDOS</div><div style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+(c.total_orders||0)+'</div></div>'
      +'<div><div style="font-family:Share Tech Mono,monospace;font-size:8px;color:'+GOLD+'">CRÉDITO</div><div style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+SOLES+(c.credit_balance||0)+'</div></div>'
      +'</div></div>';
    ...
    h+=custDetail.orders.length?custDetail.orders.map(function(o){return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:10px 14px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-body,#F2F0EB)">'+esc(o.ref)+'</div><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0)">'+esc(o.date)+' · '+SOLES+o.total+'</div></div>'+stBadge(o.status)+'</div>';}).join(''):'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0)">Sin pedidos //</div>';
    ...
    if(custDetail.ratings&&custDetail.ratings.length){
      ...
      h+=custDetail.ratings.map(function(r){return'<div style="padding:8px 0;border-bottom:1px solid #1E3932"><span style="color:#F5C518">'+'★'.repeat(r.stars)+'</span>'+(r.comment?'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(r.comment)+'</div>':'')+'</div>';}).join('');
    }
  }
  h+='</div>';
  return h;
...
function sAdminSearch(){
  ...
  h+='<div style="margin:10px 0"><select id="so-status" style="width:100%;background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#0d0d0d);border-radius:10px;padding:14px 16px;color:var(--sw-text,#FFFFFF);font-size:14px">'
  ...
  if(searchResults!==null){
    ...
    h+=searchResults.length?searchResults.map(function(o,i){
      return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px;margin-bottom:10px">'
        +'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px"><div><div style="font-family:Barlow Condensed,sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(o.customer_name||'Invitado')+'</div><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0)">'+esc(o.ref)+' · '+esc(o.date)+' · '+SOLES+o.total+'</div></div>'+stBadge(o.status)+'</div>'
        +((o.contact_phone||o.customer_phone)?'<button onclick="waSearchResult('+i+')" style="all:unset;cursor:pointer;font-family:Barlow Condensed,sans-serif;font-size:11px;color:'+GOLD+'">'+iconTxt('chat','WhatsApp',GOLD)+'</button>':'')
        +'</div>';
    }).join(''):'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);text-align:center;padding:20px 0">Sin resultados //</div>';
  }
  h+='</div>';
  return h;
}
...
var AUDIT_ACTION_LABEL={
  ...
  'set-store-hours':'Horario de atención editado','update-status':'Estado de pedido actualizado',
...
function sAdminAudit(){
  ...
  h+=log.length?log.map(function(l){
    return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:12px 14px;margin-bottom:8px">'
      +'<div style="display:flex;justify-content:space-between"><span style="font-family:Barlow Condensed,sans-serif;font-size:14px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(AUDIT_ACTION_LABEL[l.action]||l.action)+'</span><span style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0)">'+esc(l.actor_phone)+'</span></div>'
      +(l.target?'<div style="font-family:Barlow,sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px;word-break:break-all">'+esc(String(l.target))+'</div>':'')
      +'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">'+esc(new Date(l.created_at).toLocaleString('es-PE'))+'</div>'
      +'</div>';
  }).join(''):'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);text-align:center;padding:20px 0">Sin registros aún //</div>';
  h+='</div>';
  return h;
}
...
function sAdminHours(){
  ...
  h+=DOW_NAMES.map(function(name,i){
    ...
    return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:10px">'
      ...
      +'<span style="font-family:Barlow Condensed,sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+name+'</span>'
      +'<label style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);display:flex;align-items:center;gap:6px"><input type="checkbox" id="sh-closed-'+i+'" '+(d.closed?'checked':'')+' onchange="toggleClosedDay('+i+')" style="accent-color:'+GOLD+'">CERRADO</label>'
      +'</div>'
      +(d.closed?'':'<div style="display:flex;gap:8px;align-items:center"><input id="sh-open-'+i+'" type="number" min="0" max="24" value="'+(d.open==null?11:d.open)+'" style="flex:1;background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:9px 12px;color:var(--sw-text,#FFFFFF);font-family:Share Tech Mono,monospace;font-size:12px"><span style="color:var(--sw-text-muted,#A8C8B0)">a</span><input id="sh-close-'+i+'" type="number" min="0" max="24" value="'+(d.close==null?22:d.close)+'" style="flex:1;background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:9px 12px;color:var(--sw-text,#FFFFFF);font-family:Share Tech Mono,monospace;font-size:12px"></div>')
      +'</div>';
  }).join('');
...
function sAdminReport(){
  ...
  h+='<div style="display:flex;gap:8px;margin-bottom:10px">'
    +'<input id="rr-from" type="date" value="'+esc(reportFrom)+'" style="flex:1;background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#0d0d0d);border-radius:10px;padding:12px;color:var(--sw-text,#FFFFFF);font-size:16px">'
    +'<input id="rr-to" type="date" value="'+esc(reportTo)+'" style="flex:1;background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#0d0d0d);border-radius:10px;padding:12px;color:var(--sw-text,#FFFFFF);font-size:16px">'
    +'</div>';
  ...
  if(reportData){
    ...
    h+='<div style="display:flex;gap:10px;margin-bottom:16px">'
      ...
      +'</div>';
    if(d.truncated)h+='<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:#ffa500;margin-bottom:12px;display:flex;align-items:center;gap:5px">'+icon('warning',12,'#ffa500')+'<span>Hay más pedidos en este rango de los que se muestran aquí.</span></div>';
    ...
    h+=Object.keys(d.byMethod).length?Object.keys(d.byMethod).map(function(m){var v=d.byMethod[m];return DBAR(m.toUpperCase(),v.count,d.count);}).join(''):'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0)">Sin datos //</div>';
...
function sAdminRatings(){
  ...
  h+='<div style="display:flex;gap:10px;align-items:center;margin-bottom:14px;flex-wrap:wrap">'
    +'<select id="rt-min" onchange="applyRatingsFilter()" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#0d0d0d);border-radius:8px;padding:9px 12px;color:var(--sw-text,#FFFFFF);font-family:Share Tech Mono,monospace;font-size:11px">'
    +[0,1,2,3,4,5].map(function(n){return'<option value="'+n+'" '+(ratingsMinStars===n?'selected':'')+'>'+(n===0?'Todas':n+'★ o más')+'</option>';}).join('')
    +'</select>'
    +'<label style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);display:flex;align-items:center;gap:6px"><input type="checkbox" id="rt-comments" onchange="applyRatingsFilter()" '+(ratingsOnlyComments?'checked':'')+' style="accent-color:'+GOLD+'">Solo con comentario</label>'
    +'</div>';
  ...
  h+=list.length?list.map(function(r){
    return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px;margin-bottom:10px">'
      +'<div style="display:flex;justify-content:space-between"><span style="color:#F5C518;font-size:14px">'+'★'.repeat(r.stars)+'<span style="color:#3A6B58">'+'★'.repeat(5-r.stars)+'</span></span><span style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0)">'+esc(r.order_ref||'')+'</span></div>'
      +(r.comment?'<div style="font-family:Barlow,sans-serif;font-size:13px;color:var(--sw-text-body,#F2F0EB);margin-top:6px">'+esc(r.comment)+'</div>':'')
      ...
      +(r.customer_phone?'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:'+GOLD+';margin-top:6px">'+esc(r.customer_phone)+'</div>':'')
      +'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:6px">'+esc(new Date(r.created_at).toLocaleDateString('es-PE'))+'</div>'
      +'</div>';
  }).join(''):'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);text-align:center;padding:20px 0">Sin calificaciones //</div>';
  h+='</div>';
  return h;
}
...
function sAdminPrepList(){
  ...
  if(shortfalls.length){
    h+='<div style="background:rgba(255,85,85,.12);border:1px solid rgba(255,85,85,.35);border-radius:10px;padding:14px 16px;margin-bottom:16px"><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:#ff8888;letter-spacing:.1em;margin-bottom:6px;display:flex;align-items:center;gap:5px">'+icon('warning',12,'#ff8888')+'<span>NO VA A ALCANZAR //</span></div>'
      +shortfalls.map(function(i){return'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-body,#F2F0EB);margin-bottom:4px">'+esc(i.label)+' — necesitas '+i.qty+(i.stockQty!=null?', tienes '+i.stockQty:', sin stock')+'</div>';}).join('')
      +'</div>';
  }
  ...
  h+=d.ingredients.length?d.ingredients.map(function(i){
    return'<div style="display:flex;justify-content:space-between;align-items:center;background:var(--sw-card,#2D5246);border:1px solid '+(i.shortfall?'rgba(255,85,85,.4)':'#3A6B58')+';border-radius:8px;padding:10px 14px;margin-bottom:8px"><span style="font-family:Barlow,sans-serif;font-size:13px;color:var(--sw-text-body,#F2F0EB)">'+esc(i.label)+'</span><span style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:900;color:'+(i.shortfall?'#ff8888':GOLD)+'">×'+i.qty+'</span></div>';
  }).join(''):'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:16px">Sin pedidos programados en esta ventana.</div>';
  if(d.orders.length){
    ...
    h+=d.orders.map(function(o){
      return'<div style="font-family:Share Tech Mono,monospace;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:6px">'+esc(o.ref)+' · '+esc(o.customerName)+' · '+esc(new Date(o.deliveryTime).toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'}))+'</div>';
    }).join('');
  }
...
function mktBlock(pkg,week){
  return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#1c1c1c);border-radius:10px;padding:16px;margin-bottom:14px">'
    +'<div style="font-family:Barlow Condensed,sans-serif;font-size:16px;font-weight:700;color:var(--sw-text,#FFFFFF);margin-bottom:12px">'+esc(pkg.theme)+'</div>'
    ...
    +'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-body,#F2F0EB);line-height:1.5;margin-bottom:6px">'+esc(pkg.whatsapp)+'</div>'
    ...
    +'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-body,#F2F0EB);line-height:1.5;margin-bottom:6px">'+esc(pkg.caption)+'</div>'
    ...
    +'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);line-height:1.5">'+esc(pkg.photoIdea)+'</div>'
    +'</div>';
}
function sAdminMarketing(){
  ...
  h+='<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:16px;line-height:1.5">Contenido listo para copiar y pegar — cambia cada semana. Nada se publica solo, tú decides cuándo y dónde.</div>';
  ...
  h+='<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.2em;margin-bottom:8px">PRÓXIMA SEMANA //</div>';
...
function sAdminTimeReport(){
  ...
  h+=d.hours.length?d.hours.map(function(hr){
    ...
    return'<div style="background:var(--sw-card,#2D5246);border:1px solid '+(urgent?'rgba(255,85,85,.4)':'#3A6B58')+';border-radius:10px;padding:12px 14px;margin-bottom:8px"><div style="display:flex;justify-content:space-between;align-items:center"><span style="font-family:Barlow Condensed,sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+String(hr.hour).padStart(2,'0')+':00–'+String((hr.hour+1)%24).padStart(2,'0')+':00</span><span style="font-family:Share Tech Mono,monospace;font-size:13px;color:'+(urgent?'#ff8888':GOLD)+'">'+hr.cancelRatePct+'% cancelado</span></div><div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">'+hr.total+' pedido'+(hr.total===1?'':'s')+' · '+hr.cancelled+' cancelado'+(hr.cancelled===1?'':'s')+(hr.avgDeliveryMin!=null?' · entrega prom. '+hr.avgDeliveryMin+' min':'')+'</div></div>';
  }).join(''):'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:16px">Sin pedidos en este período.</div>';
...
function sAdminProblemAddresses(){
  ...
  h+=addrs.length?addrs.map(function(a){
    return'<div style="background:var(--sw-card,#2D5246);border:1px solid rgba(255,85,85,.3);border-radius:10px;padding:14px;margin-bottom:10px"><div style="display:flex;justify-content:space-between;align-items:flex-start"><span style="font-family:Barlow,sans-serif;font-size:13px;color:var(--sw-text-body,#F2F0EB);flex:1">'+esc(a.address)+'</span><span style="font-family:Barlow Condensed,sans-serif;font-size:18px;font-weight:900;color:#ff8888;flex-shrink:0;margin-left:10px">'+a.cancelCount+'</span></div>'
      +(a.reasons&&a.reasons.length?'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-top:6px">'+a.reasons.map(function(r){return esc(r);}).join(' · ')+'</div>':'')
      +'</div>';
  }).join(''):'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:16px">Sin direcciones con cancelaciones repetidas.</div>';
...
function sAdminComplaints(){
  ...
  h+='<select onchange="setComplaintsFilter(this.value)" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#0d0d0d);border-radius:8px;padding:9px 12px;color:var(--sw-text,#FFFFFF);font-family:Share Tech Mono,monospace;font-size:11px;margin-bottom:14px">'
  ...
  h+=list.length?list.map(function(c){
    ...
    return'<div style="background:var(--sw-card,#2D5246);border:1px solid '+(pending?'rgba(255,165,0,.35)':'#3A6B58')+';border-radius:10px;padding:14px;margin-bottom:10px">'
      ...
      +'<div style="font-family:Barlow Condensed,sans-serif;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(c.claim_code)+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+(c.kind==='queja'?'QUEJA':'RECLAMO')+'</div>'
      +'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(c.consumer_name)+' · '+esc(c.consumer_phone)+' · '+esc(c.consumer_email)+'</div>'
      +'</div><span style="font-family:Share Tech Mono,monospace;font-size:9px;color:'+(pending?'#ffa500':'#25D366')+';flex-shrink:0;display:inline-flex;align-items:center;gap:4px">'+(pending?icon('horario',11,'#ffa500')+'<span>PENDIENTE</span>':'✓ ATENDIDO')+'</span></div>'
      +'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-body,#F2F0EB);margin-top:8px;line-height:1.5"><b>Detalle:</b> '+esc(c.detail)+'</div>'
      +'<div style="font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px;line-height:1.5"><b>Pide:</b> '+esc(c.consumer_request)+'</div>'
      +(c.order_ref?'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:6px">Pedido: '+esc(c.order_ref)+'</div>':'')
      +'<div style="font-family:Share Tech Mono,monospace;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:6px">'+esc(new Date(c.created_at).toLocaleDateString('es-PE'))+'</div>'
      +(c.provider_response?'<div style="background:var(--sw-card2,#1A3028);border-radius:8px;padding:10px 12px;margin-top:10px;font-family:Barlow,sans-serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0)"><b style="color:'+GOLD+'">Respuesta:</b> '+esc(c.provider_response)+'</div>'
        :(openId
          ?'<div style="margin-top:10px"><textarea id="cq-resp-'+c.id+'" placeholder="Escribe tu respuesta al consumidor" style="background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:10px 12px;color:var(--sw-text,#FFFFFF);width:100%;font-size:12px;font-family:Barlow,sans-serif;min-height:70px;box-sizing:border-box;margin-bottom:8px"></textarea><button onclick="doRespondComplaint(\''+c.id+'\')" style="all:unset;cursor:pointer;display:block;width:100%;background:'+GOLD+';color:#fff;font-family:Barlow Condensed,sans-serif;font-size:12px;font-weight:700;letter-spacing:.06em;padding:10px 0;border-radius:8px;text-align:center">GUARDAR RESPUESTA //</button></div>'
      ...
      +'</div>';
  }).join(''):'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:var(--sw-text-muted,#A8C8B0);text-align:center;padding:20px 0">Sin reclamaciones //</div>';
  h+='</div>';
  return h;
}
...
function sPRecover(){
  var pinBox=recNewPin?'<div style="background:var(--sw-card2,#1A3028);border:2px solid '+GOLD+';border-radius:12px;padding:20px;margin-bottom:16px;text-align:center"><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:8px">TU NUEVO PIN //</div><div onclick="togglePinReveal()" style="cursor:pointer;font-family:Barlow Condensed,sans-serif;font-size:36px;font-weight:900;color:'+GOLD+(recPinRevealed?'':';filter:blur(9px);user-select:none')+'">'+recNewPin+'</div><div onclick="togglePinReveal()" style="cursor:pointer;font-family:Share Tech Mono,monospace;font-size:9px;color:'+GOLD+';letter-spacing:.1em;margin-top:6px;display:flex;align-items:center;justify-content:center;gap:5px">'+icon(recPinRevealed?'lock':'camera',11,GOLD)+(recPinRevealed?'OCULTAR':'TOCA PARA VER')+'</div><div style="font-family:Barlow,sans-serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-top:8px">Guárdalo — úsalo para ingresar con tu teléfono. No dejes esta pantalla abierta en un dispositivo compartido.</div></div>'
  ...
  return H('RECUPERAR CUENTA',"sc='p_auth';render()")
    ...
    +'<div style="font-family:Barlow Condensed,sans-serif;font-size:22px;font-weight:900;color:var(--sw-text-body,#F2F0EB);margin-bottom:6px">RECUPERAR PIN //</div>'
    +'<div style="font-family:Barlow,sans-serif;font-size:13px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:24px;line-height:1.5">Verifica tu identidad con tu teléfono, DNI y fecha de nacimiento. Si tienes correo registrado, te mandamos el PIN nuevo ahí; si no, te lo mostramos aquí mismo.</div>'
    +pinBox
    +'<div style="display:flex;flex-direction:column;gap:10px;margin-bottom:16px">'
    +INP('rec-phone','TELÉFONO // 9XXXXXXXX','tel',recPhone,'phone')
    +INP('rec-dni','DNI // Tu número de 8 dígitos','text',recDni,'card')
    +INP('rec-bday','FECHA NACIMIENTO // DD/MM/AAAA','text',recBday,'calendar')
    +'</div>'
