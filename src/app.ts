...
function paymentMethodPickerHTML(t){
  ...
  return'<div style="margin-top:16px"><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:8px">¿CÓMO PAGAS? //</div><div style="display:flex;gap:8px">'
    ...
    +'</div>'
    +(culqiConfigured?'<div style="font-family:\'Barlow\',sans-serif;font-size:10px;color:'+GOLD+';margin-top:6px">✨ Paga con Yape/Plin y gana +10% de puntos extra</div>':'')
...
window.culqi=function(){
  ...
  if(_pendingWeeklyPlan){
    if(Culqi.token){
      ...
      wpMsg=(Culqi.error&&(Culqi.error.user_message||Culqi.error.merchant_message))||'No se pudo procesar el pago. Intenta con otra tarjeta.';
...
async function chargeAndFinalizeWeeklyPlan(culqiToken){
  ...
  try{
    ...
    var data=await resp.json().catch(function(){return{};});
    if(!resp.ok||!data.success){
      busy=false;wpMsg=data.error||'El pago fue rechazado. Intenta con otra tarjeta.';_pendingWeeklyPlan=null;render();
      return;
    }
    try{
