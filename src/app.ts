...
function lowStockNote(code){
  ...
  return'<span style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:8px;color:#ffa500;margin-left:6px;white-space:nowrap">¡QUEDAN '+q+'!</span>';
}
...
function renderOverlays(){
  ...
  if(confirmState){
    html+='<div style="position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:410;display:flex;align-items:flex-end;justify-content:center" class="fi">'
      ...
      +'<button onclick="resolveConfirm(true)" style="all:unset;cursor:pointer;display:block;width:100%;background:'+GOLD+';color:#fff;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:14px;font-weight:700;letter-spacing:.08em;padding:14px;border-radius:10px;text-align:center;margin-bottom:8px;box-sizing:border-box">CONFIRMAR //</button>'
  ...
  if(promptState){
    html+='<div style="position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:420;display:flex;align-items:flex-end;justify-content:center" class="fi">'
      ...
      +'<button onclick="submitPrompt()" style="all:unset;cursor:pointer;display:block;width:100%;background:'+GOLD+';color:#fff;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:14px;font-weight:700;letter-spacing:.08em;padding:14px;border-radius:10px;text-align:center;margin-bottom:8px;box-sizing:border-box">ACEPTAR //</button>'
...
function WORDMARK(size,hero?){
  return'<span style="font-family:\'Fraunces\',serif;font-optical-sizing:auto;font-size:'+size+'px;font-weight:620;color:#fff;letter-spacing:.02em;line-height:1'+(hero?';text-shadow:0 2px 14px rgba(0,0,0,.35)':'')+'">SND<span class="wm-mark" aria-hidden="true"><i></i><i></i></span>WCH</span>';
}
...
function contactFooterHTML(){
  ...
  return'<div style="margin-top:28px;padding-top:20px;border-top:1px solid var(--sw-bg,#1E3932)">'
    ...
    +'<span onclick="bkTo=\'o_home\';sc=\'p_legal\';render()" style="cursor:pointer;font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.06em;text-decoration:underline">TÉRMINOS //</span>'
    +'<span onclick="bkTo=\'o_home\';sc=\'p_returns\';render()" style="cursor:pointer;font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.06em;text-decoration:underline">CAMBIOS Y DEVOLUCIONES //</span>'
    +'<span onclick="bkTo=\'o_home\';sc=\'p_complaints\';cmplStep=\'form\';render()" style="cursor:pointer;font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.06em;text-decoration:underline">LIBRO DE RECLAMACIONES //</span>'
    +'</div>'
...
function AB(t,can?,bk?,nfn?,nl?){
  var bb=bk?'<button onclick="'+bk+'" style="all:unset;cursor:pointer;border:1px solid var(--sw-border,#3A6B58);color:var(--sw-text-muted,#A8C8B0);font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:12px;font-weight:700;letter-spacing:.1em;padding:12px 16px;border-radius:8px;flex-shrink:0">← ATRÁS</button>':'';
  var tt=t?'<div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:8px;color:'+GOLD+';letter-spacing:.2em">TOTAL //</div><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:24px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+SOLES+'<span style="color:'+GOLD+'">'+t+'</span></div>':'';
  ...
  return'<div style="position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:480px;background:rgba(11,11,11,.97);border-top:1px solid var(--sw-border-soft,#0B0B0B);box-shadow:0 -8px 24px rgba(0,0,0,.4);padding:12px 20px;display:flex;gap:10px;align-items:center;z-index:100"><div style="flex:1">'+tt+'</div>'+bb+'<button onclick="'+(can?nfn:'')+'" '+(can?'':'disabled')+' style="all:unset;cursor:'+(can?'pointer':'not-allowed')+';background:'+(can?surfaceGrad('#E0BC7A',GOLD):'#1E3932')+';box-shadow:'+(can?'0 3px 10px rgba(203,162,88,.3)':'none')+';color:'+(can?'#241a08':'#4A7A68')+';font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:14px;font-weight:700;letter-spacing:.1em;padding:13px 0;border-radius:8px;text-align:center;flex:1">'+(nl||'CONTINUAR //')+'</button></div>';
}
...
function CARD(item,sel,fn,right?,thumb?){
  var inner='<div style="display:flex;justify-content:space-between;align-items:center"><span style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:16px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+item.l+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+item.s+'</span>'+(right||'')+'</div>'+(item.d?'<p style="font-family:\'EB Garamond\',serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">'+item.d+'</p>':'');
...
function ST(n,t,s?){return'<div style="margin-bottom:20px"><h2 style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:28px;font-weight:900;color:#fff;letter-spacing:.06em;line-height:1">'+n+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+t+'</h2>'+(s?'<p style="font-family:\'EB Garamond\',serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:5px">'+s+'</p>':'')+'</div>';}
...
function sOHome(){
  ...
  return H()
    ...
    +cartCard
    +recoCard
    +'<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:var(--sw-text-body,#F2F0EB);opacity:.8;letter-spacing:.15em;margin-bottom:20px">\xbfCÓMO QUIERES PEDIR? //</div>'
    ...
    +'<div style="position:absolute;right:16px;top:16px;font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:#0d0d0d;background:'+GOLD+';border-radius:4px;padding:3px 8px;letter-spacing:.1em">RECOMENDADO</div>'
    +(function(){
      ...
      return'<div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:27px;font-weight:900;color:#fff;margin-bottom:8px;text-wrap:balance">SIGNATURE<span class="cut-sep" style="color:'+GOLD+'"> // </span>BUILDS</div>'
      +'<p style="font-family:EB Garamond,serif;font-size:13px;color:var(--sw-text-muted,#A8C8B0);line-height:1.5">'+visibleSigs.length+' combinaciones curadas. Rápido, sin decisiones.</p>'
      +'<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:14px">'+visibleSigs.map(function(s){var nw=s.badge==='NUEVO';return'<span style="font-family:EB Garamond,serif;font-style:italic;font-size:9px;color:'+(nw?'#25D366':GOLD)+';background:'+(nw?'rgba(37,211,102,.1)':'rgba(203,162,88,.1)')+';border:1px solid '+(nw?'rgba(37,211,102,.25)':'rgba(203,162,88,.25)')+';border-radius:20px;padding:3px 10px">'+s.n+'</span>';}).join('')+'</div>';
    })()
    +'</div>'
    ...
    +'<div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:26px;font-weight:900;color:#fff;margin-bottom:8px">BUILD<span class="cut-sep" style="color:'+GOLD+'"> // </span>YOUR OWN</div>'
    +'<p style="font-family:EB Garamond,serif;font-size:13px;color:var(--sw-text-muted,#A8C8B0);line-height:1.5">Elige base, proteína, toppings y salsas.</p>'
    +'</div>'
    +(cust?'<div onclick="doCreateGroupOrder()" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:12px;padding:14px 16px;display:flex;justify-content:space-between;align-items:center;cursor:pointer;margin-bottom:16px"><div><div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">PEDIDO<span class="cut-sep" style="color:'+GOLD+'"> // </span>GRUPAL</div><div style="font-family:EB Garamond,serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">Para la oficina — cada quien agrega su sándwich, pagas todo junto</div></div><span style="font-family:EB Garamond,serif;font-style:italic;font-size:11px;color:'+GOLD+'">ORGANIZAR →</span></div>':'')
    +(cust&&myFavorites.length?'<div onclick="sc=\'p_favorites\';render()" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:12px;padding:14px 16px;display:flex;justify-content:space-between;align-items:center;cursor:pointer;margin-bottom:16px"><span style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF);display:inline-flex;align-items:center;gap:6px">'+icon('estrella',15,'#FFFFFF')+'<span>MIS<span class="cut-sep" style="color:'+GOLD+'"> // </span>FAVORITOS</span></span><span style="font-family:EB Garamond,serif;font-style:italic;font-size:11px;color:'+GOLD+'">VER \u2192</span></div>':'')
    +pc
    +contactFooterHTML()
    +'</div>'+NAV();
}
...
function sGroupOrder(){
  ...
  h+='<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:24px;font-weight:900;color:#fff;margin-bottom:4px">PEDIDO<span class="cut-sep" style="color:'+GOLD+'"> // </span>GRUPAL</div>';
...
function sOSig(){
  ...
  var h=H('SIGNATURE BUILDS','go(\'o_home\')',true)+'<div style="flex:1;padding:20px 20px 140px;overflow-y:auto" class="fi">'+SZTOG()+recoCardSig+ST('01','ELIGE TU BUILD','Tres salsas incluidas.')+SIGS.map(function(s){
    ...
    return'<div onclick="sigId=\''+s.id+'\';cheese=null;render()" style="background:'+(sel?surfaceGrad('#24543F','#173327'):surfaceGrad('#1E3A30','#162922'))+';border:1px solid '+(sel?GOLD:'#3A6B58')+';border-radius:10px;padding:16px;cursor:pointer;margin-bottom:10px;position:relative;transition:all .15s;box-shadow:'+(sel?SHADOW_GOLD:SHADOW_SM)+'"><div style="display:flex;gap:14px">'+thumb+'<div style="flex:1;min-width:0">'+selBar(sel)+'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:6px"><span style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:18px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+s.n+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+sigTypeTag(s.s)+'</span><span style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:14px;color:'+(sel?GOLD:'#444')+';margin-left:12px">'+priceTag+'</span></div><span style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:'+GOLD+';background:rgba(203,162,88,.12);border:1px solid rgba(203,162,88,.35);border-radius:4px;padding:2px 7px">'+s.badge+'</span>'+(s.chef?'<span style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:#0d0d0d;background:'+GOLD+';border-radius:4px;padding:2px 7px;margin-left:6px">FAVORITO DEL CHEF</span>':'')+lowStockNote(s.prot)+pitchPreview+ingredientsLine+'<div onclick="event.stopPropagation();openSigPreview(\''+s.id+'\')" style="margin-top:10px;display:inline-flex;align-items:center;gap:5px;font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.1em;cursor:pointer">'+icon('camera',11,GOLD)+'VER FOTO →</div></div></div></div>';
  }).join('');
  var sig=SIGS.find(function(x){return x.id===sigId;});
...
function sigPreviewOverlayHTML(){
  ...
  var hero=photo
    ...
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:24px;font-weight:900;color:#fff">'+s.n+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+sigTypeTag(s.s)+'</div>'
    +'<span style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:9px;color:'+GOLD+';background:rgba(203,162,88,.12);border:1px solid rgba(203,162,88,.35);border-radius:4px;padding:2px 8px;margin-top:8px;display:inline-block">'+s.badge+'</span>'
    +'</div>';
  return'<div onclick="closeSigPreview()" style="position:fixed;inset:0;background:rgba(0,0,0,.75);z-index:200;display:flex;align-items:flex-end;justify-content:center" class="fi">'
    ...
    +'<button onclick="closeSigPreview();sigId=\''+s.id+'\';go(\'o_sig\')" style="all:unset;cursor:pointer;display:block;width:100%;background:'+GOLD+';color:#fff;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:14px;font-weight:700;letter-spacing:.1em;padding:14px;border-radius:10px;text-align:center;margin-bottom:8px">PEDIR ESTE SIGNATURE //</button>'
...
function comboDrinkNudgeHTML(){
  ...
  return isOffPeakDrinkPromoActiveNow()
    ?'<div style="font-family:\'EB Garamond\',serif;font-size:11px;color:'+GOLD+';margin-top:14px">Es hora valle — agrega una bebida y te sale GRATIS (hasta '+SOLES+OFFPEAK_DRINK_PROMO_CAP+')</div>'
    :'<div style="font-family:\'EB Garamond\',serif;font-size:11px;color:'+GOLD+';margin-top:14px">Agrega una bebida y ahorra '+SOLES+COMBO_DISCOUNT_PER_PAIR+' (combo)</div>';
}
...
function deliveryZonePickerHTML(){
  ...
  h+=DELIVERY_PRICE_ZONES.map(function(z){
    ...
    return'<div onclick="deliveryZone=\''+z.id+'\';confirmRerender()" style="flex:1;min-width:110px;text-align:center;background:'+(sel?surfaceGrad('#24543F','#173327'):surfaceGrad('#1E3A30','#162922'))+';border:1px solid '+(sel?GOLD:'#3A6B58')+';border-radius:8px;padding:10px 8px;cursor:pointer"><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:12px;font-weight:700;color:'+(sel?'#fff':'#A8C8B0')+'">'+z.l+'</div><div style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:11px;color:'+(sel?GOLD:'#A8C8B0')+';margin-top:2px">'+SOLES_TXT+z.fee+'</div></div>';
  }).join('');
...
function payMethodBtn(id,label,enabled,badge){
  ...
  return'<div onclick="'+(enabled?'selectPayMethod(\''+id+'\')':'')+'" style="flex:1;text-align:center;background:'+(sel?surfaceGrad('#24543F','#173327'):surfaceGrad('#1E3A30','#162922'))+';border:1px solid '+(sel?GOLD:'#3A6B58')+';border-radius:8px;padding:10px 6px;cursor:'+(enabled?'pointer':'not-allowed')+';opacity:'+(enabled?1:.4)+'">'
    ...
    +'<div style="display:flex;align-items:center;justify-content:center;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:13px;font-weight:700;color:#fff">'+label+'</div>'
...
function sOSent(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px;text-align:center;background:var(--sw-bg,#1E3932)" class="fi">'
    ...
    +'<button onclick="sc=\'o_home\';render()" style="all:unset;cursor:pointer;border:1px solid '+GOLD+';color:'+GOLD+';font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:13px;font-weight:700;letter-spacing:.15em;padding:12px 22px;border-radius:10px">NUEVO PEDIDO</button>'
    +(cust?'<button onclick="sc=\'p_orders\';loadMyOrders()" style="all:unset;cursor:pointer;background:'+GOLD+';color:#0d0d0d;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:13px;font-weight:700;letter-spacing:.15em;padding:12px 22px;border-radius:10px">VER ESTADO →</button>':'')
    +'</div>'
    +(!cust?'<div onclick="atab=\'reg\';sc=\'p_auth\';render()" style="margin-top:20px;cursor:pointer;font-family:\'EB Garamond\',serif;font-weight:600;font-size:10px;color:'+GOLD+';letter-spacing:.1em">→ CREAR CUENTA Y GANAR PUNTOS POR ESTE PEDIDO</div>':'')
    +'</div>';
}
...
function sWelcome(){
  ...
  return'<div onclick="sc=\'p_home\';render()" style="min-height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;background:var(--sw-bg,#1E3932);padding:48px 24px;position:relative;overflow:hidden">'
    ...
    +'<div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:10px;font-weight:700;color:'+GOLD+';letter-spacing:.28em;margin-bottom:10px">BIENVENIDO //</div>'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:48px;font-weight:900;color:var(--sw-text-body,#F2F0EB);line-height:1;margin-bottom:4px">'+esc(nm.toUpperCase())+'</div>'
    ...
    +'<div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:10px;color:'+GOLD+';letter-spacing:.2em;font-weight:700;margin-bottom:12px">TUS PUNTOS //</div>'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:72px;font-weight:900;color:'+GOLD+';line-height:1">'+pts+'</div>'
    +'<div style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:4px">PUNTOS ACUMULADOS</div>'
    +(rwd?'<div style="margin-top:12px;background:rgba(203,162,88,.15);border-radius:8px;padding:8px 12px"><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:14px;font-weight:700;color:'+GOLD+'">✓ PUEDES CANJEAR: '+(rwd.n+' '+rwd.s).toUpperCase()+'</div></div>':'')
    +(next?'<div style="margin-top:8px"><div style="font-family:\'EB Garamond\',serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0)">Te faltan <span style="color:var(--sw-text-body,#F2F0EB);font-weight:700">'+(next.pts-pts)+' pts</span> para '+next.n+' // '+next.s+'</div></div>':'')
    +'</div>'
    ...
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:var(--sw-text-body,#F2F0EB)">'+(cust?cust.total_orders||0:0)+'</div>'
    ...
    +'<div style="background:rgba(242,240,235,.08);border:1px solid rgba(242,240,235,.12);border-radius:12px;padding:12px 16px;text-align:center">'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:var(--sw-text-body,#F2F0EB)">'+(cust?cust.total_redeemed||0:0)+'</div>'
    ...
    +'</div>'
    +'<div style="margin-top:20px;font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:var(--sw-text-body,#F2F0EB);letter-spacing:.15em;opacity:.7" class="blink">toca para continuar //</div>'
    +'</div></div>';
}
function sPHome(){
...
function sPOrders(){
  ...
  function card(o){
    ...
    return'<div onclick="_od=\''+o.id+'\';rtStars=0;rtMsg=\'\';sc=\'p_ord_detail\';render()" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:12px;padding:14px;margin-bottom:10px;cursor:pointer">'
      ...
      +'<div><div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:16px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(o.customer_name)+'</div>'
      ...
      +'<div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:20px;font-weight:900;color:'+GOLD+';flex-shrink:0">'+SOLES+o.total+'</div></div>'
...
function sOrdDetail(){
  ...
  return H('DETALLE',"sc='p_orders';render()")+'<div style="flex:1;padding:20px 20px 40px;overflow-y:auto" class="fi">'
    ...
    +'<div><div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+esc(o.customer_name)+'</div>'
    ...
    +'<div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:34px;font-weight:900;color:'+GOLD+'">'+SOLES+o.total+'</div></div>'
...
function sGiftCard(){
  ...
  return H('TARJETA DE REGALO','sc=\'p_profile\';render()')+'<div style="flex:1;padding:24px 20px 140px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:#fff;margin-bottom:4px">TARJETA<span class="cut-sep" style="color:'+GOLD+'"> // </span>DE REGALO</div>'
...
function sWeeklyPlan(){
  return H('PLAN SEMANAL','sc=\'p_profile\';render()')+'<div style="flex:1;padding:24px 20px 140px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:#fff;margin-bottom:4px">PLAN<span class="cut-sep" style="color:'+GOLD+'"> // </span>SEMANAL</div>'
...
function sPLegal(){
  ...
  return H('TÉRMINOS Y PRIVACIDAD',"sc='"+bk+"';render()")+'<div style="flex:1;padding:24px 20px 40px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:#fff;margin-bottom:4px">TÉRMINOS<span class="cut-sep" style="color:'+GOLD+'"> // </span>Y PRIVACIDAD</div>'
...
function sPReturns(){
  ...
  return H('CAMBIOS Y DEVOLUCIONES',"sc='"+bk+"';render()")+'<div style="flex:1;padding:24px 20px 40px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:#fff;margin-bottom:4px">CAMBIOS<span class="cut-sep" style="color:'+GOLD+'"> // </span>Y DEVOLUCIONES</div>'
    +'<div style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:20px">Última actualización: 2026</div>'
    +providerBlockHTML()
...
function sPComplaints(){
  ...
  var kindToggle='<div style="display:flex;background:var(--sw-card,#2D5246);border-radius:10px;padding:4px;margin-bottom:20px">'+[['reclamo','RECLAMO'],['queja','QUEJA']].map(function(x){return'<button onclick="cmplKind=\''+x[0]+'\';render()" style="all:unset;cursor:pointer;flex:1;background:'+(cmplKind===x[0]?GOLD:'transparent')+';color:'+(cmplKind===x[0]?'#fff':'#555')+';font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:13px;font-weight:700;letter-spacing:.1em;padding:11px 0;border-radius:8px;text-align:center;transition:all .15s">'+x[1]+'</button>';}).join('')+'</div>';
  ...
  return H('LIBRO DE RECLAMACIONES',"sc='"+bk+"';render()")+'<div style="flex:1;padding:24px 20px 40px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:#fff;margin-bottom:4px">LIBRO DE<span class="cut-sep" style="color:'+GOLD+'"> // </span>RECLAMACIONES</div>'
...
function sComplaintsSuccess(bk){
  return H('LIBRO DE RECLAMACIONES',"sc='"+bk+"';render()")+'<div style="flex:1;padding:24px 20px 40px;overflow-y:auto;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center" class="fi">'
    ...
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:20px;font-weight:900;color:#fff;margin-bottom:8px">'+(cmplKind==='queja'?'QUEJA':'RECLAMO')+' REGISTRAD'+(cmplKind==='queja'?'A':'O')+'</div>'
    ...
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:32px;font-weight:900;color:'+GOLD+';margin-bottom:20px">'+esc(cmplCode||'')+'</div>'
...
function sPAddresses(){
  ...
  if(myAddresses.length){
    h+=myAddresses.map(function(a){
      return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:8px;display:flex;justify-content:space-between;align-items:center"><div><div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(a.label)+'</div><div style="font-family:EB Garamond,serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-top:2px">'+esc(a.address)+'</div></div><button onclick="doDeleteAddress(\''+a.id+'\')" style="all:unset;cursor:pointer;color:#ff8888;font-family:EB Garamond,serif;font-style:italic;font-size:10px;flex-shrink:0;margin-left:10px">ELIMINAR</button></div>';
    }).join('');
  }else{
...
function bulkBar(){
  ...
  return'<div style="position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:480px;background:rgba(11,11,11,.97);border-top:1px solid var(--sw-border-soft,#0B0B0B);padding:12px 16px;display:flex;gap:6px;align-items:center;z-index:110">'
    ...
    +'<button onclick="bulkConfirmPayments()" style="all:unset;box-sizing:border-box;cursor:pointer;flex:1;text-align:center;background:#ffa500;color:#0d0d0d;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:12px;font-weight:700;padding:15px 4px;border-radius:8px">'+iconTxt('check','PAGO','#0d0d0d')+'</button>'
    +'<button onclick="bulkAdvanceStatus(\'PREPARANDO\')" style="all:unset;box-sizing:border-box;cursor:pointer;flex:1;text-align:center;background:'+STATUSES.PREPARANDO.c+';color:#fff;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:12px;font-weight:700;padding:15px 4px;border-radius:8px">PREPARANDO</button>'
    +'<button onclick="bulkAdvanceStatus(\'EN CAMINO\')" style="all:unset;box-sizing:border-box;cursor:pointer;flex:1;text-align:center;background:'+STATUSES['EN CAMINO'].c+';color:#fff;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:12px;font-weight:700;padding:15px 4px;border-radius:8px">EN CAMINO</button>'
    +'<button onclick="bulkAdvanceStatus(\'ENTREGADO\')" style="all:unset;box-sizing:border-box;cursor:pointer;flex:1;text-align:center;background:'+STATUSES.ENTREGADO.c+';color:#fff;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:12px;font-weight:700;padding:15px 4px;border-radius:8px">ENTREGADO</button>'
...
function sAdminHome(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;background:var(--sw-bg,#1E3932)">'
    ...
    +'<div><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:24px;font-weight:900;color:var(--sw-text,#FFFFFF)">PANEL<span class="cut-sep" style="color:'+GOLD+'"> // </span>OPERADOR</div>'
    +(badge>0?'<div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:#ffa500;letter-spacing:.1em;margin-top:3px" class="pulse">● '+badge+' ACCIÓN REQUERIDA</div>':'<div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:var(--sw-text-muted,#A8C8B0);margin-top:3px">todo en orden //</div>')
    ...
    +'<div><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:20px;font-weight:900;color:#F5C518">PANEL<span style="color:#fff"> // </span>DE NEGOCIO</div><div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:#a08010;letter-spacing:.1em;margin-top:2px">ventas · productos top · clientes · puntos</div></div>'
    +'<span style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:12px;color:#F5C518">VER →</span></div>'
    ...
    +(ao.length?ao.map(function(o){
      ...
      return'<div style="background:var(--sw-card,#2D5246);border:1px solid '+(isStale?'#ff5555':(o.status==='RECIBIDO'?'rgba(255,165,0,.3)':'#1c1c1c'))+';border-radius:10px;padding:16px;margin-bottom:12px">'
        ...
        +'<div style="flex:1"><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:17px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(o.customer_name)+'</div>'
        ...
        +'<div style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:9px;color:'+(isStale?'#ff8888':'#A8C8B0')+';margin-top:4px;display:flex;align-items:center;gap:5px">'+(isStale?'<span class="pulse" style="width:6px;height:6px;border-radius:50%;background:#ff5555;display:inline-block;flex-shrink:0"></span>':'')+'<span>'+esc(o.ref)+' · '+SOLES+o.total+' · '+esc(o.date)+(mins!==null?' · hace '+mins+' min':'')+'</span></div>'
        ...
        +'<button onclick="cancelOrder(\''+o.id+'\')" style="all:unset;cursor:pointer;display:block;width:100%;background:transparent;border:1px solid rgba(255,85,85,.4);color:#ff8888;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:12px;font-weight:700;letter-spacing:.06em;padding:9px 0;border-radius:8px;text-align:center">'+iconTxt('close','CANCELAR PEDIDO'+(manualPending?' (nunca pagó)':''),'#ff8888')+'</button>'
        +'</div>';
    ...
    +'</select></div>'
    +'<div style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:9px;color:#222;text-align:center;margin-top:6px">Auto-actualiza cada 25 seg · Sonido al recibir pedido</div>'
    +'</div>'
    +bulkBar()
    +'</div>';
}
...
function sAdminDashboard(){
  ...
  // Ventas
  h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">VENTAS //</div>';
  ...
  if(d.estimatedProfit){
    h+='<div style="background:rgba(37,211,102,.08);border:1px solid rgba(37,211,102,.25);border-radius:10px;padding:14px 16px;margin-bottom:18px">'
      ...
      +[['HOY',d.estimatedProfit.today],['SEMANA',d.estimatedProfit.week],['MES',d.estimatedProfit.month],['TOTAL',d.estimatedProfit.allTime]].map(function(x){
        return'<div><div style="font-family:EB Garamond,serif;font-weight:600;font-size:8px;color:var(--sw-text-muted,#A8C8B0);letter-spacing:.1em">'+x[0]+'</div><div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:16px;font-weight:900;color:#25D366">'+SOLES+x[1].low+'–'+SOLES+x[1].high+'</div></div>';
      }).join('')
      +'</div></div>';
  }
  ...
  if(d.trend&&d.trend.length){
    ...
    h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">TENDENCIA · 14 DÍAS //</div>';
  ...
  h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">PEDIDOS POR ESTADO //</div>';
  if(!stEntries.length){
  ...
  // Productos top
  h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">PRODUCTOS MÁS VENDIDOS //</div>';
  ...
  h+='<div style="height:1px;background:var(--sw-bg,#1E3932);margin:16px 0"></div>';
  // Clientes
  h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">CLIENTES //</div>';
  ...
  h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">CLIENTES EN RIESGO //</div>';
  ...
  h+='<div style="height:1px;background:var(--sw-bg,#1E3932);margin:16px 0"></div>';
  // Puntos
  h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">PUNTOS //</div>';
  h+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:8px">'
    ...
    +'</div>';
  h+='<div style="height:1px;background:var(--sw-bg,#1E3932);margin:16px 0"></div>';
  h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">EXPORTAR //</div>';
...
// ADMIN GEN
function sAdminGen(){return'<div style="min-height:100vh;display:flex;flex-direction:column;background:var(--sw-bg,#1E3932)"><div style="padding:20px;border-bottom:1px solid #3A6B58;display:flex;align-items:center;gap:12px"><button onclick="sc=\'admin_home\';render()" style="all:unset;cursor:pointer;color:var(--sw-text-muted,#A8C8B0);font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:18px;font-weight:700;padding:0 12px 0 0">←</button><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:var(--sw-text,#FFFFFF)">PUNTOS<span class="cut-sep" style="color:'+GOLD+'"> // </span>MANUALES</div></div><div style="flex:1;padding:24px 20px" class="fi"><p style="font-family:\'EB Garamond\',serif;font-size:14px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:20px;line-height:1.6">Otorga puntos confirmados directamente a un cliente.</p><div style="display:flex;flex-direction:column;gap:12px;margin-bottom:16px">'+INP('ag-ph','TELÉFONO DEL CLIENTE // 9XXXXXXXX','tel',agPhone,'phone')+INP('ag-pts','PUNTOS A OTORGAR // Ej: 25','number',agPts)+BTN('OTORGAR PUNTOS //','doManualPts()')+'</div><div id="ag-msg" style="font-family:\'EB Garamond\',serif;font-size:13px;color:'+GOLD+';min-height:20px">'+agMsg+'</div>'
  +'<div style="height:1px;background:var(--sw-card,#2D5246);margin:28px 0 24px"></div>'
  +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:18px;font-weight:900;color:var(--sw-text,#FFFFFF);margin-bottom:4px">CRÉDITO<span class="cut-sep" style="color:'+GOLD+'"> // </span>AJUSTE MANUAL</div>'
...
function render(){
  ...
  var offlineBanner=isOffline?'<div style="background:#ffa500;color:#1a1200;text-align:center;padding:6px;font-family:\'EB Garamond\',serif;font-weight:600;font-size:10px;letter-spacing:.1em;font-weight:700;display:flex;align-items:center;justify-content:center;gap:5px">'+icon('warning',12,'#1a1200')+'<span>SIN CONEXIÓN — reconectando…</span></div>':'';
...
function sAdminInventory(){
  ...
  INV_CATS.forEach(function(cat){
    h+='<div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:18px;font-weight:900;color:var(--sw-text,#FFFFFF);margin:18px 0 10px">'+cat.t+'<span class="cut-sep" style="color:'+GOLD+'"> //</span></div>';
    h+=cat.arr.map(function(item){
      ...
      return'<div class="inv-row" data-name="'+esc(name.toLowerCase())+'" style="background:'+(av?'var(--sw-card,#2D5246)':'var(--sw-card-danger,#1A2420)')+';border:1px solid '+(av?'var(--sw-border,#3A6B58)':'rgba(255,85,85,.3)')+';border-radius:10px;padding:13px 16px;margin-bottom:8px">'
        ...
        +'<div><div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:15px;font-weight:700;color:'+(av?'var(--sw-text,#FFFFFF)':'var(--sw-text-muted,#A8C8B0)')+'">'+name+'</div>'
...
function cpRow(label,inputsHtml,fn){
  return'<div class="cp-row" data-name="'+esc(label.toLowerCase())+'" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:10px">'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF);margin-bottom:10px">'+esc(label)+'</div>'
    ...
    +'<button onclick="'+fn+'" style="all:unset;cursor:pointer;background:'+GOLD+';color:#fff;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:12px;font-weight:700;padding:9px 14px;border-radius:8px;white-space:nowrap">GUARDAR</button></div></div>';
}
...
function sAdminCustomer(){
  ...
  if(custDetail){
    ...
    h+='<div style="height:1px;background:var(--sw-bg,#1E3932);margin:20px 0"></div>'
      ...
      +'<div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:20px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+esc(c.name)+'</div>'
      ...
      +'<div><div style="font-family:EB Garamond,serif;font-style:italic;font-size:8px;color:'+GOLD+'">PUNTOS</div><div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:18px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+(c.points||0)+'</div></div>'
      +'<div><div style="font-family:EB Garamond,serif;font-style:italic;font-size:8px;color:'+GOLD+'">PEDIDOS</div><div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:18px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+(c.total_orders||0)+'</div></div>'
      +'<div><div style="font-family:EB Garamond,serif;font-style:italic;font-size:8px;color:'+GOLD+'">CRÉDITO</div><div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:18px;font-weight:900;color:var(--sw-text,#FFFFFF)">'+SOLES+(c.credit_balance||0)+'</div></div>'
      +'</div></div>';
...
function sAdminHours(){
  ...
  h+=DOW_NAMES.map(function(name,i){
    ...
    return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border,#3A6B58);border-radius:10px;padding:14px 16px;margin-bottom:10px">'
      ...
      +'<span style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+name+'</span>'
...
function sAdminRatings(){
  ...
  h+='<div style="display:flex;gap:10px;align-items:center;margin-bottom:14px;flex-wrap:wrap">'
    +'<select id="rt-min" onchange="applyRatingsFilter()" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#0d0d0d);border-radius:8px;padding:9px 12px;color:var(--sw-text,#FFFFFF);font-family:EB Garamond,serif;font-style:italic;font-size:11px">'
...
function sAdminPrepList(){
  ...
  h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">INGREDIENTES A PREPARAR //</div>';
  ...
  if(d.orders.length){
    ...
    h+='<div style="font-family:EB Garamond,serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:10px;font-weight:700">PEDIDOS INCLUIDOS //</div>';
...
function mktBlock(pkg,week){
  return'<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#1c1c1c);border-radius:10px;padding:16px;margin-bottom:14px">'
    +'<div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:16px;font-weight:700;color:var(--sw-text,#FFFFFF);margin-bottom:12px">'+esc(pkg.theme)+'</div>'
    ...
    +'<button onclick="copyMktText(\''+week+'\',\'whatsapp\')" style="all:unset;cursor:pointer;background:'+GOLD+';color:#0d0d0d;font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:10px;font-weight:700;padding:7px 12px;border-radius:6px;margin-bottom:14px;display:inline-block">COPIAR</button>'
    ...
    +'<button onclick="copyMktText(\''+week+'\',\'caption\')" style="all:unset;cursor:pointer;background:'+GOLD+';color:#0d0d0d;font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:10px;font-weight:700;padding:7px 12px;border-radius:6px;margin-bottom:14px;display:inline-block">COPIAR</button>'
...
function sAdminComplaints(){
  ...
  h+='<select onchange="setComplaintsFilter(this.value)" style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#0d0d0d);border-radius:8px;padding:9px 12px;color:var(--sw-text,#FFFFFF);font-family:EB Garamond,serif;font-style:italic;font-size:11px;margin-bottom:14px">'
  ...
  h+=list.length?list.map(function(c){
    ...
    return'<div style="background:var(--sw-card,#2D5246);border:1px solid '+(pending?'rgba(255,165,0,.35)':'#3A6B58')+';border-radius:10px;padding:14px;margin-bottom:10px">'
      ...
      +'<div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:15px;font-weight:700;color:var(--sw-text,#FFFFFF)">'+esc(c.claim_code)+'<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+(c.kind==='queja'?'QUEJA':'RECLAMO')+'</div>'
      ...
      +(c.provider_response?'<div style="background:var(--sw-card2,#1A3028);border-radius:8px;padding:10px 12px;margin-top:10px;font-family:EB Garamond,serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0)"><b style="color:'+GOLD+'">Respuesta:</b> '+esc(c.provider_response)+'</div>'
        :(openId
          ?'<div style="margin-top:10px"><textarea id="cq-resp-'+c.id+'" placeholder="Escribe tu respuesta al consumidor" style="background:var(--sw-card2,#1A3028);border:1px solid var(--sw-border,#3A6B58);border-radius:8px;padding:10px 12px;color:var(--sw-text,#FFFFFF);width:100%;font-size:12px;font-family:EB Garamond,serif;min-height:70px;box-sizing:border-box;margin-bottom:8px"></textarea><button onclick="doRespondComplaint(\''+c.id+'\')" style="all:unset;cursor:pointer;display:block;width:100%;background:'+GOLD+';color:#fff;font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:12px;font-weight:700;letter-spacing:.06em;padding:10px 0;border-radius:8px;text-align:center">GUARDAR RESPUESTA //</button></div>'
          :'<button onclick="cmplRespondingId=\''+c.id+'\';render()" style="all:unset;cursor:pointer;display:block;width:100%;text-align:center;background:rgba(203,162,88,.12);border:1px solid rgba(203,162,88,.4);color:'+GOLD+';font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:12px;font-weight:700;letter-spacing:.06em;padding:9px 0;border-radius:8px;margin-top:10px">RESPONDER //</button>'))
      +'</div>';
...
function sPRecover(){
  ...
  return H('RECUPERAR CUENTA',"sc='p_auth';render()")
    ...
    +'<div style="font-family:Bodoni Moda,serif;font-optical-sizing:auto;font-size:22px;font-weight:900;color:var(--sw-text-body,#F2F0EB);margin-bottom:6px">RECUPERAR PIN //</div>'
