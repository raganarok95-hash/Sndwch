...
function willPayWithCard(){
  ...
  // Solo engorda el fee si el cliente ya tocó "Tarjeta" a propósito — mientras no lo
  // haga, el total mostrado nunca lleva un recargo que no eligió, aunque el pedido
  // termine yendo por Culqi de todos modos si nunca toca el selector (mismo camino
  // rápido de siempre, solo que esos pocos pedidos salen sin el fee cubierto).
  return payMethodChosen;
}
...
// pagas?" (Yape/Plin o Tarjeta) — antes el total mostrado ya incluía la comisión de
// Culqi con solo LLEGAR al checkout, sin ninguna elección real de por medio (hallazgo P2
// de crítica impeccable 2026-07-30). Mientras esto sea false, deliveryFeeAmount() nunca
// engorda el fee, así que si el cliente paga sin tocar el selector, el pedido igual sale
// por Culqi (mismo camino rápido de siempre) pero al precio sin comisión — el negocio
// absorbe esos ~S/0.30-0.50, nunca al revés (jamás se cobra más de lo que se mostró).
...
var CAL_STATUS_LABEL={draft:'Borrador',scheduled:'Programado',posted:'Publicado'};
var CAL_STATUS_COLOR={draft:'#A8C8B0',scheduled:GOLD,posted:'#25D366'};
async function loadCalendar(){
  sc='admin_calendar';busy=true;busyMsg='Cargando calendario...';render();
...
function sAdminCalendar(){
  ...
  h+='<div style="font-family:EB Garamond,serif;font-style:italic;font-size:10px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:10px;line-height:1.5">Sube aquí el video crudo (sin editar) de lo que grabaste — una sesión programada aparte lo recorta al formato correcto, escribe el caption y lo agenda solo, sin que tengas que volver a tocar nada.</div>';
  ...
  if(rawUploads&&rawUploads.length){
    h+='<div style="margin-top:10px;font-family:EB Garamond,serif;font-size:11px;color:var(--sw-text-muted,#A8C8B0)">'+rawUploads.length+' clip'+(rawUploads.length===1?'':'s')+' esperando la próxima sesión de procesamiento.</div>';
  }
  h+='</div>';
  h+='<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#1c1c1c);border-radius:10px;padding:16px;margin-bottom:20px">';
    ...
    h+=calendarData.map(function(e){
      ...
      var photoBlock=canAutoPublish?(
        ...
        +(isVideoEntry?'':
          ...
          +'</label>')
        +(hasMedia&&e.status!=='posted'?'<span onclick="'+(publishingThis?'':'publishCalendarEntry(\''+e.id+'\')')+'" style="cursor:pointer;font-family:EB Garamond,serif;font-size:11px;color:#25D366">'+(publishingThis?'Publicando...':'Publicar ahora →')+'</span>':'')
...
async function loadWaitlist(){
  sc='admin_waitlist';busy=true;busyMsg='Cargando lista de espera...';render();
