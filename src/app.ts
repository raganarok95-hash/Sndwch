...
var STATUSES={
  'RECIBIDO':  {c:'#ffa500',next:'PREPARANDO', icon:'📋'},
  'PREPARANDO':{c:'#3A86FF',next:'EN CAMINO',  icon:''},
  'EN CAMINO': {c:'#9b6fff',next:'ENTREGADO',  icon:'🛵'},
  'ENTREGADO': {c:'#25D366',next:null,          icon:'✅'},
...
function renderOverlays(){
  ...
  if(sc.indexOf('admin')!==0){
    ...
    html+='<a href="https://wa.me/'+WA+'?text='+supportMsg+'" target="_blank" rel="noopener" style="position:fixed;right:16px;bottom:84px;z-index:150;width:50px;height:50px;border-radius:50%;background:'+GOLD+';display:flex;align-items:center;justify-content:center;box-shadow:0 4px 14px rgba(0,0,0,.4);text-decoration:none;font-size:24px" aria-label="Soporte por WhatsApp">💬</a>';
  }
  el.innerHTML=html;
}
...
function stBadge(st){var s=STATUSES[st]||STATUSES['RECIBIDO'];return'<span style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+s.c+';background:'+s.c+'18;border:1px solid '+s.c+'44;border-radius:4px;padding:3px 9px;letter-spacing:.1em">'+(s.icon?s.icon+' ':'')+st+'</span>';}
...
function H(sub?,bk?,showCart?){
  ...
  var cartIcon=(showCart&&cart.length)?'<button onclick="go(\'o_cart\')" style="all:unset;cursor:pointer;position:relative;flex-shrink:0;padding:6px 10px;background:#2D5246;border-radius:8px;font-size:18px">🛒<span style="position:absolute;top:-4px;right:2px;background:'+GOLD+';color:#0d0d0d;font-family:\'Share Tech Mono\',monospace;font-size:9px;font-weight:700;border-radius:8px;padding:1px 5px;min-width:14px;text-align:center">'+cart.reduce(function(s,it){return s+it.qty;},0)+'</span></button>':'';
  return'<div style="padding:20px 20px 16px;border-bottom:1px solid #3A6B58;display:flex;align-items:center;flex-shrink:0">'+b+'<div style="flex:1"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:'+sz+'px;font-weight:900;color:#fff;letter-spacing:.06em;line-height:1">SND<span style="color:'+GOLD+'">//</span>WCH</div>'+s2+'</div>'+cartIcon+'</div>';
}
function NAV(){
  var oa=tab==='order';
...
function contactFooterHTML(){
  ...
  return'<div style="margin-top:28px;padding-top:20px;border-top:1px solid #1E3932">'
    ...
    +'<div>📍 Delivery — '+BIZ_CITY+'</div>'
    +'<div>✉️ <a href="mailto:'+BIZ_EMAIL+'" style="color:#A8C8B0;text-decoration:none">'+BIZ_EMAIL+'</a></div>'
    +'<div>💬 <a href="https://wa.me/'+WA+'" target="_blank" rel="noopener" style="color:#A8C8B0;text-decoration:none">+51 930 957 640</a></div>'
    +'</div>'
...
function BTN(l,fn,out?){return'<button onclick="'+fn+'" style="all:unset;box-sizing:border-box;cursor:pointer;display:block;width:100%;background:'+(out?'transparent':GOLD)+';border:'+(out?'1px solid '+GOLD:'none')+';color:'+(out?GOLD:'#fff')+';font-family:\'Barlow Condensed\',sans-serif;font-size:14px;font-weight:700;letter-spacing:.1em;padding:14px;border-radius:10px;text-align:center">'+l+'</button>';}
function LOAD(msg){return'<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#1E3932"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:38px;font-weight:900;color:#fff;letter-spacing:.08em;margin-bottom:16px">SND<span style="color:'+GOLD+'">//</span>WCH</div><div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:#A8C8B0;letter-spacing:.25em">'+(msg||'CARGANDO //')+'</div></div>';}
...
function sigPreviewOverlayHTML(){
  ...
  var hero=photo
    ...
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:24px;font-weight:900;color:#fff">'+s.n+'<span style="color:'+GOLD+'"> // </span>'+s.s+'</div>'
...
function sOItemConfirm(){
  ...
  var recU=(!doubleProt&&dbl)?{k:'doubleProt',e:'💪',l:'DOBLE PROTEÍNA',d:'El doble de tu proteína elegida',p:dbl.pDbl}:!extraSauce?{k:'sauce',e:'🌶️',l:'SALSA EXTRA',d:'Salsa adicional a tu elección',p:2}:null;
  ...
  var sigNameHTML=(mode==='sig'&&sig)?'<div style="margin:2px 0 14px"><div style="font-family:\'Barlow Condensed\',sans-serif;font-size:26px;font-weight:900;color:#FFFFFF;letter-spacing:.03em;line-height:1.15">'+sig.n+'<span style="color:'+GOLD+'"> // </span>'+sig.s+'</div></div>':'';
  return H('CONFIRMAR SÁNDWICH',(quickPayEligible?'backFromConfirm()':'go(\''+bk+'\')'),true)+'<div style="flex:1;padding:20px 20px 110px;overflow-y:auto" class="fi">'
    ...
    +'<details style="margin-bottom:12px"><summary style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;cursor:pointer;font-weight:700;list-style:none;padding:8px 0">TODOS LOS EXTRAS // ▾</summary><div style="margin-top:8px">'+(dbl?uBtn('doubleProt','💪','DOBLE PROTEÍNA','El doble de tu proteína elegida',dbl.pDbl,doubleProt):'')+uBtn('sauce','🌶️','SALSA EXTRA','Salsa adicional a tu elección',2,extraSauce)+'</div></details>'
...
function checkoutExtrasHTML(){
  ...
  return pBox
    ...
    +(!cust||pushSubscribed||!cust.total_orders||!('serviceWorker' in navigator)||!('PushManager' in window)?'':'<div onclick="togglePushNotifications()" style="margin-top:16px;background:#1A3028;border:1px solid rgba(203,162,88,.3);border-radius:10px;padding:14px 16px;cursor:pointer"><div style="font-family:\'Barlow\',sans-serif;font-size:12px;color:#F2F0EB;line-height:1.4">🔔 Te notificamos del estado de tu pedido<span style="color:'+GOLD+'"> — </span><span style="color:'+GOLD+';font-weight:700">actívalo aquí →</span></div>'+(pushMsg?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:10px;color:'+GOLD+';margin-top:6px">'+esc(pushMsg)+'</div>':'')+'</div>');
}
// Selector de método de pago (tarjeta/Yape/Plin) — se oculta si el pedido ya se cubre
// por completo con crédito interno. Mientras Culqi no esté configurado (ver
// CULQI_PUBLIC_KEY), TARJETA ni siquiera se muestra — un botón de pago marcado
// "PRONTO" se lee como una app a medio terminar; mejor mostrar solo los métodos
// que sí funcionan hoy (Yape/Plin).
function paymentMethodPickerHTML(t){
  var culqiConfigured=CULQI_PUBLIC_KEY&&CULQI_PUBLIC_KEY.indexOf('REEMPLAZA')<0;
  // Antes había un botón "YAPE" y otro "PLIN" por separado, pero ambos llevan a la
  // misma pantalla de instrucciones (mismo número, mismo botón de copiar) — obligar al
  // cliente a elegir entre los dos no le da ninguna información nueva, solo un tap de
  // más. Un solo botón cubre ambas apps; internamente sigue mandando 'yape' al servidor
  // (el backend solo distingue "pago manual pendiente de confirmar" de todo lo demás,
  // nunca trató Yape y Plin como cosas distintas más allá de esa etiqueta).
...
function doOrder(){
  ...
  var summary=itemSummaries.join(' · ')+(rewardObj?' · 🎁 '+rewardObj.n+' '+rewardObj.s:'');
...
window.culqi=function(){
  ...
  if(Culqi.token){
    ...
    busy=false;_payingInProgress=false;render();
    var msg=(Culqi.error&&(Culqi.error.user_message||Culqi.error.merchant_message))||'No se pudo procesar el pago. Intenta con otra tarjeta.';
    var errEl=(document.getElementById('o-err') as HTMLInputElement | null);
...
async function chargeAndFinalize(culqiToken){
  ...
  try{
    ...
    if(!resp.ok||!data.success){
      busy=false;_payingInProgress=false;render();
      var errEl=(document.getElementById('o-err') as HTMLInputElement | null);
      var msg=data.error||'El pago fue rechazado. Intenta con otra tarjeta.';
      if(errEl)errEl.textContent=msg;else showToast(msg);
      return;
    }
...
function sOSent(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:32px;text-align:center;background:#1E3932" class="fi">'
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:52px;font-weight:900;color:#fff;letter-spacing:.08em;line-height:1;margin-bottom:12px">SND<span style="color:'+GOLD+'">//</span>WCH</div>'
...
async function doReg(){
  ...
  try{
    ...
    var r=await api('register',{name:name,phone:phone,pin:pin,email:email||null,dni:dni,bday:bday,referredBy:referredBy||null,claimOrderRef:claimRef});
...
function sWelcome(){
  ...
  return'<div onclick="sc=\'p_home\';render()" style="min-height:100vh;display:flex;flex-direction:column;justify-content:center;align-items:center;background:#1E3932;padding:40px 24px;position:relative;overflow:hidden">'
    ...
    +'<div style="font-family:\'Share Tech Mono\',monospace;font-size:11px;color:#A8C8B0;letter-spacing:.3em;margin-bottom:16px">SND//WCH</div>'
    ...
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:14px;font-weight:700;color:'+GOLD+';letter-spacing:.2em;margin-bottom:8px">¡BIENVENIDO A SND//WCH!</div>'
    +'<div style="font-family:\'Barlow Condensed\',sans-serif;font-size:46px;font-weight:900;color:#F2F0EB;line-height:1;margin-bottom:4px">'+esc(nm.toUpperCase())+'</div>'
    +'<div style="width:60px;height:3px;background:'+GOLD+';margin:16px auto 24px;border-radius:2px"></div>'
    ...
    +[['🛒','Gana puntos con cada pedido pagado.'],['🎁','Canjéalos por salsas, upgrades y sándwiches gratis.'],['💌','Invita amigos — ambos ganan 50 puntos extra.']].map(function(x){return'<div style="display:flex;gap:10px;align-items:flex-start;margin-bottom:8px"><span style="font-size:15px">'+x[0]+'</span><span style="font-family:\'Barlow\',sans-serif;font-size:12px;color:#A8C8B0;line-height:1.4">'+x[1]+'</span></div>';}).join('')
    +'</div>'
...
function sPOrders(){
  ...
  if(listLoading){
    ...
    if(!myOrders.length){
      h+='<div style="text-align:center;padding-top:64px"><div style="font-size:32px;margin-bottom:12px;opacity:.5">🧾</div><div style="font-family:Share Tech Mono,monospace;font-size:10px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:12px">SIN PEDIDOS //</div>'+BTN('HACER UN PEDIDO //','swTab(\'order\')')+'</div>';
    }
  }
...
function sOrdDetail(){
  ...
  return H('DETALLE',"sc='p_orders';render()")+'<div style="flex:1;padding:20px 20px 40px;overflow-y:auto" class="fi">'
    ...
    +(o.redeemed_reward?'<div style="margin-top:10px;padding-top:10px;border-top:1px solid #3A6B58"><div style="font-family:Share Tech Mono,monospace;font-size:9px;color:#25D366">🎁 '+esc(o.redeemed_reward)+'</div></div>':'')
    +'</div>'
...
function sPHistory(){
  ...
  if(listLoading){
    ...
    h+='<div style="text-align:center;padding-top:64px"><div style="font-size:32px;margin-bottom:12px;opacity:.5">⭐</div><div style="font-family:Share Tech Mono,monospace;font-size:10px;color:'+GOLD+';letter-spacing:.2em">SIN MOVIMIENTOS //</div></div>';
  }else{
...
function sPAddresses(){
  ...
  if(myAddresses.length){
    ...
    h+='<div style="text-align:center;padding-top:40px;margin-bottom:20px"><div style="font-size:32px;margin-bottom:12px;opacity:.5">📍</div><div style="font-family:Share Tech Mono,monospace;font-size:10px;color:'+GOLD+';letter-spacing:.2em">SIN DIRECCIONES GUARDADAS //</div></div>';
  }
...
// Íconos de línea minimalistas para el grid de herramientas del panel admin — mismo
// trazo/estilo que el ícono de Instagram del pie de página (stroke currentColor, sin
// relleno), en vez de emoji grandes y de colores dispares que no calzan con la estética
// tipográfica del resto de la app.
var ADMIN_ICONS={
...
async function loadDashboard(){
  ...
  try{dashStats=await api('dashboard-stats',{token:token});}
  catch(e){dashStats=null;}
  busy=false;render();
}
