...
// Selector de método de pago (tarjeta/Yape automático vía Culqi, o Yape/Plin manual
// por transferencia) — se oculta si el pedido ya se cubre por completo con crédito
// interno. Mientras Culqi no esté configurado (ver CULQI_PUBLIC_KEY), TARJETA/YAPE ni
// siquiera se muestra — un botón de pago marcado "PRONTO" se lee como una app a medio
// terminar; mejor mostrar solo los métodos que sí funcionan hoy (transferencia manual).
function paymentMethodPickerHTML(t){
  ...
  return'<div style="margin-top:16px"><div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+GOLD+';letter-spacing:.2em;margin-bottom:8px">¿CÓMO PAGAS? //</div><div style="display:flex;gap:8px">'
    +(culqiConfigured?payMethodBtn('culqi','TARJETA / YAPE',true):'')
    +payMethodBtn('yape','TRANSFERENCIA',true)
    +'</div>'
...
function sGiftCard(){
  return H('TARJETA DE REGALO','sc=\'p_profile\';render()')+'<div style="flex:1;padding:24px 20px 100px;overflow-y:auto" class="fi">'
    ...
    +'<p style="font-family:\'Barlow\',sans-serif;font-size:12px;color:#A8C8B0;margin-bottom:20px;line-height:1.5">Compra crédito SND//WCH con tarjeta o Yape y regálaselo a otro cliente al instante. Monto entre S/10 y S/500.</p>'
...
function sWeeklyPlan(){
  return H('PLAN SEMANAL','sc=\'p_profile\';render()')+'<div style="flex:1;padding:24px 20px 100px;overflow-y:auto" class="fi">'
    ...
    +'<p style="font-family:\'Barlow\',sans-serif;font-size:12px;color:#A8C8B0;margin-bottom:20px;line-height:1.5">Paga '+SOLES+WEEKLY_PLAN_PRICE+' hoy con tarjeta o Yape y recibe '+SOLES+WEEKLY_PLAN_CREDIT+' en saldo SND//WCH al instante — pide cuando quieras esta semana, el saldo no vence.</p>'
