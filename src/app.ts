...
// distinguían por el relleno, ambos en el mismo dorado. El tono original (#3A86FF)
// no llegaba a 4.5:1 de contraste sobre los fondos oscuros de la app (auditoría de
// accesibilidad) — este es más claro para pasar WCAG AA en texto normal manteniendo
// el mismo azul.
var WA='51930957640',GOLD='#CBA258',ACCENT2='#9BC4FF';
...
var STATUSES={
  ...
  'PREPARANDO':{c:ACCENT2,next:'EN CAMINO',  icon:''},
...
function renderOverlays(){
  ...
  if(toastMsg){
    ...
    html+='<div style="position:fixed;left:16px;right:16px;bottom:20px;z-index:400;display:flex;justify-content:center" class="fi">'
      ...
      +'<button onclick="dismissToast()" aria-label="Cerrar aviso" style="all:unset;cursor:pointer;color:'+(isErr?'#ffb3b3':'#A8C8B0')+';font-size:16px;line-height:1;padding:10px">&#10005;</button>'
      +'</div></div>';
  }
  if(confirmState){
...
function H(sub?,bk?,showCart?){
  var b=bk?'<button onclick="'+bk+'" aria-label="Volver" style="all:unset;cursor:pointer;color:#A8C8B0;font-family:\'Barlow Condensed\',sans-serif;font-size:18px;font-weight:700;padding:12px 14px 12px 0;flex-shrink:0">←</button>':'';
  var sz=sub?26:40;
  ...
  var cartIcon=(showCart&&cart.length)?'<button onclick="go(\'o_cart\')" aria-label="Ver carrito" style="all:unset;cursor:pointer;position:relative;flex-shrink:0;padding:13px 15px;background:#2D5246;border-radius:8px;display:flex">'+icon('cart',18,'#F2F0EB')+'<span style="position:absolute;top:-4px;right:2px;background:'+GOLD+';color:#0d0d0d;font-family:\'Share Tech Mono\',monospace;font-size:9px;font-weight:700;border-radius:8px;padding:1px 5px;min-width:14px;text-align:center">'+cart.reduce(function(s,it){return s+it.qty;},0)+'</span></button>':'';
...
function checkoutExtrasHTML(){
  ...
  return pBox
    ...
    +'<div style="display:flex;flex-direction:column;gap:10px;margin-top:16px">'+INP('o-nom','NOMBRE // Tu nombre','text',confNom)+INP('o-phone','TELÉFONO // 9XXXXXXXX','tel',confPhone)+INP('o-email','CORREO // Opcional, para tu comprobante','email',confEmail)+'<div style="position:relative">'+INP('o-addr','DIRECCION // Calle o usa GPS','text',addrText)+'<button id="gps-btn" onclick="doGPS()" aria-label="Usar mi ubicación GPS" style="all:unset;cursor:pointer;position:absolute;right:0;top:0;bottom:0;width:44px;display:flex;align-items:center;justify-content:center;color:#A8C8B0;font-family:Barlow,sans-serif;font-size:11px;font-weight:700">&#128205;</button></div>'+'<div id="gps-hint" style="min-height:12px;margin-top:3px"></div>'+INP('o-notes','NOTAS // opcional','text',confNotes)+'</div>'
...
function sAdminHome(){
  ...
  return'<div style="min-height:100vh;display:flex;flex-direction:column;background:#1E3932">'
    ...
    +'</div><button onclick="loadAdmin()" title="Actualizar ahora" aria-label="Actualizar ahora" style="all:unset;cursor:pointer;font-size:16px;margin-right:14px;padding:12px">🔄</button>'
    +'<button onclick="toggleAdminLight()" title="Modo claro/oscuro" aria-label="Modo claro/oscuro" style="all:unset;cursor:pointer;font-size:16px;margin-right:14px;padding:12px">'+(adminLightMode?'🌙':'☀️')+'</button>'
    ...
    +(ao.length?ao.map(function(o){
      ...
      return'<div style="background:#2D5246;border:1px solid '+(isStale?'#ff5555':(o.status==='RECIBIDO'?'rgba(255,165,0,.3)':'#1c1c1c'))+';border-radius:10px;padding:16px;margin-bottom:12px'+(isStale?'" class="pulse':'')+'">'
        ...
        +(isScheduledAhead?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+ACCENT2+';margin-top:2px">🕒 programado para '+esc(new Date(o.scheduled_for).toLocaleTimeString('es-PE',{hour:'2-digit',minute:'2-digit'}))+'</div>':'')
        ...
        +(o.status==='EN CAMINO'&&o.eta_minutes?'<div style="font-family:\'Share Tech Mono\',monospace;font-size:9px;color:'+ACCENT2+';margin-top:2px">🛵 ETA ~'+o.eta_minutes+' min</div>':'')+'</div></div>'
...
function confirmMap(){
  ...
  if(el){el.value=a;el.style.borderColor=ACCENT2;el.focus();}
...
// Accesibilidad: casi todas las "tarjetas" clicables de la app (elegir proteína,
// topping, dirección guardada, favorito para pedir de nuevo, etc.) son <div
// onclick=...> en vez de <button> — se ven y funcionan igual con mouse/touch, pero
// un lector de pantalla no las anuncia como interactivas y un usuario que solo usa
// teclado no puede llegar a ellas con Tab. Reescribir cientos de plantillas para usar
// <button> es un cambio enorme; en vez de eso, se enriquece el DOM ya renderizado en
// cada render(): todo elemento con onclick que no sea ya nativamente enfocable recibe
// tabIndex=0 + role="button", y un listener delegado traduce Enter/Espacio en un
// click real. Cubre pantallas nuevas automáticamente sin tocar cada función sN().
