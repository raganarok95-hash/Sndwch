...
var AUDIT_ACTION_LABEL={
  ...
  'calendar-image-upload':'Foto subida a una entrada del calendario','social-publish':'Publicación enviada a Meta (Instagram/Facebook)',
};
...
function sAdminCalendar(){
  ...
  h+='<div style="font-family:EB Garamond,serif;font-size:12px;color:var(--sw-text-muted,#A8C8B0);margin-bottom:16px;line-height:1.5">Planea fechas y canales reales. Instagram y Facebook sí se pueden publicar de verdad desde acá (sube una foto y toca "Publicar ahora") — el resto de canales sigue siendo copiar el texto a mano.</div>';
  h+='<div style="background:var(--sw-card,#2D5246);border:1px solid var(--sw-border-soft,#1c1c1c);border-radius:10px;padding:16px;margin-bottom:20px">';
  ...
  if(!calendarData||!calendarData.length){
    ...
    h+=calendarData.map(function(e){
      ...
      var photoBlock=canAutoPublish?(
        ...
        +'</label>'
        +(e.image_url&&e.status!=='posted'?'<span onclick="'+(publishingThis?'':'publishCalendarEntry(\''+e.id+'\')')+'" style="cursor:pointer;font-family:EB Garamond,serif;font-size:11px;color:#25D366">'+(publishingThis?'Publicando...':'Publicar ahora →')+'</span>':'')
