...
function badgesHTML(c){
  ...
  return'<div style="background:var(--sw-card2,#171A14);border:1px solid rgba(203,162,88,.25);border-radius:12px;padding:18px;margin-bottom:16px"><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:15px;font-weight:600;color:var(--sw-text,#FFFFFF);margin-bottom:12px">Insignias<span class="cut-sep" style="color:'+GOLD+'"> // </span>'+unlockedCount+'/'+badges.length+'</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">'+badges.map(function(b){
    return'<div style="background:'+(b.unlocked?'var(--sw-forest-deep,#1E4636)':'var(--sw-card2,#171A14)')+';border:1px solid '+(b.unlocked?GOLD:'var(--sw-border,#2C3228)')+';border-radius:10px;padding:10px;text-align:center;opacity:'+(b.unlocked?1:.4)+'"><div>'+icon(b.icon,22,(b.unlocked?GOLD:'#666'))+'</div><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:11px;font-weight:600;color:'+(b.unlocked?'#FFFFFF':'#9DA096')+';margin-top:4px">'+b.label+'</div>'+(b.sub?'<div style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:8px;color:var(--sw-text-muted,#9DA096);margin-top:2px">'+b.sub+'</div>':'')+'</div>';
  }).join('')+'</div></div>';
}
...
function sPProfile(){
  ...
  var heroHTML='<div style="background:linear-gradient(135deg,#1B1F18,#12150F);border:1px solid var(--sw-border,#2C3228);border-radius:20px;padding:22px;margin-bottom:16px;display:flex;align-items:center;gap:16px"><div style="flex:0 0 auto;width:56px;height:56px;border-radius:50%;background:'+GOLD+';display:flex;align-items:center;justify-content:center;font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:28px;font-weight:640;color:#12241D">'+initial+'</div><div style="flex:1;min-width:0"><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:640;color:var(--sw-text,#FFFFFF);line-height:1.1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+esc(cust.name)+'</div><div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:11px;color:var(--sw-text-muted,#9DA096);margin-top:4px">'+esc(cust.phone)+'</div><div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:'+GOLD+';letter-spacing:.15em;margin-top:6px">'+rankName(cust.total_orders)+' //</div></div><div style="flex:0 0 auto;text-align:center;background:rgba(0,0,0,.2);border-radius:10px;padding:8px 12px"><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:22px;font-weight:640;color:'+GOLD+';line-height:1">'+(cust.points||0)+'</div><div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:8px;color:var(--sw-text-muted,#9DA096);letter-spacing:.1em;margin-top:2px">Pts</div></div></div>';
  var referralHTML='<div style="background:var(--sw-card2,#171A14);border:1px solid rgba(203,162,88,.25);border-radius:12px;padding:18px;margin-bottom:16px"><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:15px;font-weight:600;color:var(--sw-text,#FFFFFF);margin-bottom:10px">Programa<span class="cut-sep" style="color:'+GOLD+'"> // </span>referidos</div><div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:28px;font-weight:640;color:var(--sw-text,#FFFFFF);margin-bottom:4px">'+cust.phone+'</div><div style="font-family:\'EB Garamond\',serif;font-size:13px;color:var(--sw-text-muted,#9DA096);margin-bottom:12px">Tu código de referido · '+(cust.total_referrals||0)+' amigos referidos</div>'+referralLadderHTML(cust.total_referrals)+'<button onclick="shareReferral()" style="all:unset;cursor:pointer;display:block;width:100%;background:'+GOLD+';color:var(--sw-on-gold,#241a08);font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:13px;font-weight:600;letter-spacing:.08em;padding:12px;border-radius:8px;text-align:center">Compartir por WhatsApp //</button></div>';
  ...
  weeklyPlanHTML=weeklyPlanHTML.replace('Plan<span class="cut-sep"','<span style="float:right;font-family:\'EB Garamond\',serif;font-style:italic;font-size:9px;color:var(--sw-text-muted,#9DA096);letter-spacing:.04em">Pago con tarjeta</span>Plan<span class="cut-sep"');
...
function sGiftCard(){
  ...
  var lejos=saldo<minPts?'<div style="background:var(--sw-card2,#171A14);border:1px solid var(--sw-border,#2C3228);border-radius:10px;padding:14px 16px;margin-bottom:14px">'
    +'<div style="font-family:\'EB Garamond\',serif;font-size:13px;color:var(--sw-text-body,#EFEDE4);line-height:1.5">Todavía no te alcanza para el regalo más chico: te faltan <b style="color:'+GOLD+'">'+(minPts-saldo)+' pts</b>'
  ...
  var chips=sug.length?lejos+'<div style="display:flex;gap:8px;margin-bottom:14px">'+sug.map(function(x){
    ...
    return'<div onclick="giftElegirMonto('+x.v+')" style="flex:1;cursor:pointer;text-align:center;padding:12px 6px;border-radius:10px;'
      +'background:'+(sel?'rgba(203,162,88,.14)':'var(--sw-card,#1B1F18)')+';border:1px solid '+(sel?GOLD:'var(--sw-border,#2C3228)')+';'
      +'opacity:'+(puede?'1':'.45')+'">'
      +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:18px;font-weight:640;color:'+(sel?GOLD:'var(--sw-text,#FFFFFF)')+';line-height:1">'+SOLES_TXT+pz(x.v)+'</div>'
      +'<div style="font-family:\'EB Garamond\',serif;font-size:11px;color:var(--sw-text-muted,#9DA096);margin-top:4px">'+x.t+'</div>'
      +'<div style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:9px;color:'+(puede?GOLD:'var(--sw-text-muted,#9DA096)')+';letter-spacing:.08em;margin-top:3px">'+pts+' PTS</div>'
      +'</div>';
  }).join('')+'</div>':'';
  ...
  var resto=ptsCost>0?'<div style="background:var(--sw-card2,#171A14);border-radius:10px;padding:12px 14px;margin-bottom:12px;display:flex;justify-content:space-between;align-items:center">'
    +'<div><div style="font-family:\'EB Garamond\',serif;font-size:11px;color:var(--sw-text-muted,#9DA096)">Cuesta</div>'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:18px;font-weight:640;color:'+(alcanza?GOLD:'var(--sw-danger-strong,#ff5555)')+';line-height:1.15">'+ptsCost+' pts</div></div>'
    +'<div style="text-align:right"><div style="font-family:\'EB Garamond\',serif;font-size:11px;color:var(--sw-text-muted,#9DA096)">'+(alcanza?'Te quedan':'Te faltan')+'</div>'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:18px;font-weight:640;color:var(--sw-text,#FFFFFF);line-height:1.15">'+Math.abs(saldo-ptsCost)+' pts</div></div>'
    +'</div>':'';
  ...
  var explica='<div style="border:1px solid var(--sw-border,#2C3228);border-radius:10px;padding:14px 16px;margin-top:18px">'
    ...
    +'<div style="font-family:\'EB Garamond\',serif;font-size:13px;color:var(--sw-text-body,#EFEDE4);line-height:1.55">Le llega el saldo <b>al instante</b>, listo para usar en cualquier pedido. No vence, no hay que canjear ningún código, y si no le alcanza para todo el pedido paga la diferencia normal.</div>'
    +'<div style="font-family:\'EB Garamond\',serif;font-style:italic;font-size:11px;color:var(--sw-text-muted,#9DA096);margin-top:8px;line-height:1.45">Si todavía no tiene cuenta, el saldo lo espera: se le acredita con su número apenas se registre.</div>'
    +'</div>';

  return H('TARJETA DE REGALO','sndScreen=\'p_profile\';render()')+'<div style="flex:1;padding:24px 20px 140px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:18px;font-weight:640;color:#fff;margin-bottom:4px;text-wrap:balance">Tarjeta<span class="cut-sep" style="color:'+GOLD+'"> // </span>de regalo</div>'
    +'<p style="font-family:\'EB Garamond\',serif;font-size:13px;color:var(--sw-text-muted,#9DA096);margin-bottom:6px;line-height:1.5">Convierte tus puntos en un sándwich para alguien más. Entre '+SOLES_TXT+pz(GIFT_CARD_AMOUNT_MIN)+' y '+SOLES_TXT+pz(GIFT_CARD_AMOUNT_MAX)+'.</p>'
    +'<p style="font-family:\'EB Garamond\',serif;font-weight:600;font-size:11px;color:'+GOLD+';letter-spacing:.05em;margin-bottom:16px">Tienes '+saldo+' pts // '+GIFT_CARD_POINTS_PER_SOL+' pts = '+SOLES_TXT+pz(1)+'</p>'
    +chips
...
async function doGiftCardBuy(){
  ...
  if(!(await showConfirm('¿Regalar '+SOLES_TXT+pz(amt)+' de crédito a '+name+' ('+phone+') por '+ptsCost+' puntos?')))return;
...
function sWeeklyPlan(){
  return H('PLAN SEMANAL','sndScreen=\'p_profile\';render()')+'<div style="flex:1;padding:24px 20px 140px;overflow-y:auto" class="fi">'
    +'<div style="font-family:\'Bodoni Moda\',serif;font-optical-sizing:auto;font-size:18px;font-weight:640;color:#fff;margin-bottom:4px;text-wrap:balance">Plan<span class="cut-sep" style="color:'+GOLD+'"> // </span>semanal</div>'
    +'<p style="font-family:\'EB Garamond\',serif;font-size:13px;color:var(--sw-text-muted,#9DA096);margin-bottom:20px;line-height:1.5">Paga '+SOLES+pz(WEEKLY_PLAN_PRICE)+' hoy con tu tarjeta y recibe '+SOLES+pz(WEEKLY_PLAN_CREDIT)+' en saldo SND//WCH al instante — pide cuando quieras esta semana, el saldo no vence.</p>'
    // ── ETIQUETA ──────────────────────────────────────────────────────────────────────
...
async function doWeeklyPlanBuy(){
  ...
  if(!(await showConfirm('¿Pagar '+SOLES_TXT+pz(WEEKLY_PLAN_PRICE)+' y recibir '+SOLES_TXT+pz(WEEKLY_PLAN_CREDIT)+' en saldo?')))return;
...
async function chargeAndFinalizeWeeklyPlan(culqiToken){
  ...
  try{
    ...
    showToast('¡Listo! Recibiste '+SOLES+pz(pw.creditAmount)+' en saldo.');
    sndScreen='p_profile';render();
  }catch(e){
...
async function doClaimChallenge(){
  ...
  try{
    ...
    chalMsg='¡Reto completado! +'+CHALLENGE_BONUS_POINTS+' pts';
...
async function doClaimDiscoveryChallenge(){
  ...
  try{
    ...
    if(res.customer){cust=res.customer;cacheCust(cust,isAdmin);}
    discChalMsg='¡Reto completado! +'+DISCOVERY_BONUS_POINTS+' pts';
