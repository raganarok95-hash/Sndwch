...
import { unaBebida, unaProteina, unaSalsa } from "./carta.ts";
...
Deno.test("una tanda dentro de su vida útil no alerta", () => {
  const r = batchExpiryStatus(
    [{ product_code: INSUMO, product_name: "Res // Asado", stock_qty: 12, batch_cooked_at: haceHoras(10), shelf_life_days: 3 }],
    AHORA,
  );
...
Deno.test("una tanda pasada de su fecha límite sale como VENCIDA", () => {
  const r = batchExpiryStatus(
    [{ product_code: INSUMO, product_name: "Res // Asado", stock_qty: 12, batch_cooked_at: haceHoras(80), shelf_life_days: 3 }],
    AHORA,
  );
  assertEquals(r.vencidos.length, 1);
  assertEquals(r.vencidos[0].code, INSUMO);
...
Deno.test("avisa un día antes, no el mismo día que vence", () => {
  ...
  const dentro = batchExpiryStatus(
    [{ product_code: OTRO_INSUMO, stock_qty: 5, batch_cooked_at: haceHoras(52), shelf_life_days: 3 }],
    AHORA,
  );
  ...
  const fuera = batchExpiryStatus(
    [{ product_code: OTRO_INSUMO, stock_qty: 5, batch_cooked_at: haceHoras(46), shelf_life_days: 3 }],
    AHORA,
  );
...
Deno.test("el borde exacto de las 24 h entra en el aviso", () => {
  const r = batchExpiryStatus(
    [{ product_code: OTRO_INSUMO, stock_qty: 5, batch_cooked_at: haceHoras(3 * 24 - BATCH_EXPIRY_WARN_HOURS), shelf_life_days: 3 }],
    AHORA,
  );
...
Deno.test("un insumo SIN tanda registrada nunca alerta", () => {
  ...
  const r = batchExpiryStatus(
    [{ product_code: UNA_BEBIDA, product_name: "Limonada", stock_qty: 30, batch_cooked_at: null, shelf_life_days: 3 }],
    AHORA,
  );
  assertEquals(r.vencidos.length, 0);
  assertEquals(r.porVencer.length, 0);
...
Deno.test("un insumo sin stock no alerta aunque su tanda sea vieja — ya se consumió", () => {
  ...
  const cero = batchExpiryStatus([{ product_code: INSUMO, stock_qty: 0, batch_cooked_at: haceHoras(200), shelf_life_days: 3 }], AHORA);
  assertEquals(cero.vencidos.length, 0);

  const nulo = batchExpiryStatus([{ product_code: INSUMO, stock_qty: null, batch_cooked_at: haceHoras(200), shelf_life_days: 3 }], AHORA);
  assertEquals(nulo.vencidos.length, 0);

  const marcado = batchExpiryStatus(
    [{ product_code: INSUMO, stock_qty: 8, in_stock: false, batch_cooked_at: haceHoras(200), shelf_life_days: 3 }],
    AHORA,
  );
...
Deno.test("sin vida útil configurada usa el default conservador de 3 días", () => {
  ...
  for (const vida of [null, undefined, 0, -1, NaN] as (number | null | undefined)[]) {
    const r = batchExpiryStatus([{ product_code: INSUMO, stock_qty: 4, batch_cooked_at: haceHoras(80), shelf_life_days: vida }], AHORA);
...
Deno.test("una vida útil más larga corre la fecha límite", () => {
  ...
  const r = batchExpiryStatus([{ product_code: UNA_SALSA, stock_qty: 4, batch_cooked_at: haceHoras(80), shelf_life_days: 7 }], AHORA);
  assertEquals(r.vencidos.length, 0);
  assertEquals(r.porVencer.length, 0);
});
...
Deno.test("una fecha de tanda corrupta se ignora en vez de tumbar el cron", () => {
  ...
  const r = batchExpiryStatus(
    [
      ...
      { product_code: INSUMO, stock_qty: 4, batch_cooked_at: haceHoras(80), shelf_life_days: 3 },
    ],
    AHORA,
  );
  assertEquals(r.vencidos.length, 1);
