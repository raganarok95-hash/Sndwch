-- Decisión de negocio: ya no existe un tier VIP con beneficios distintos — todos los
-- clientes ganan los mismos puntos por sol gastado. Este RPC calculaba el conteo de
-- clientes por tier de forma independiente al tierName() de catalog.ts (ya retirado),
-- así que hay que actualizarlo aquí también para que el panel admin no siga mostrando
-- un bucket "VIP" que ya no representa ningún trato diferenciado.
...
begin
  select json_build_object(
    ...
    'referrals', (
      select json_build_object(
        ...
        'revenue', coalesce((
          ...
          where o.payment_status = 'paid' and o.customer_phone in (select phone from customers where referred_by is not null)
        ), 0)
      )
    ...
    'weekPrev', (
      select json_build_object('revenue', coalesce(sum(total), 0), 'count', count(*))
      from orders where payment_status = 'paid' and created_at >= v_prev_week_start and created_at < p_week_start
    ),
    'monthPrev', (
      select json_build_object('revenue', coalesce(sum(total), 0), 'count', count(*))
      from orders where payment_status = 'paid' and created_at >= v_prev_month_start and created_at < p_month_start
    ),
    'peakHours', (
