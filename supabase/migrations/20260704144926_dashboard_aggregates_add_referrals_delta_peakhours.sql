-- Agrega 3 métricas nuevas pedidas por la auditoría de analítica: ROI de referidos,
-- comparación automática vs. período anterior, y horas/días pico — todo calculado en SQL
-- sobre toda la tabla (no la ventana de 14 días que ya cubre el trend en el edge function).
create or replace function public.dashboard_aggregates(p_week_start timestamptz, p_month_start timestamptz)
returns json
language plpgsql
security definer
